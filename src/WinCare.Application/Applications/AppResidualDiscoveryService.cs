...
public sealed class AppResidualDiscoveryService : IAppResidualDiscoveryService
{
    ...
    public async Task<AppResidualDiscoveryReport> DiscoverAsync(AppResidualDiscoveryRequest request, CancellationToken cancellationToken)
    {
        ...
        string localLow = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "..", "LocalLow");
        var searchRoots = new[]
        {
            Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),
            Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData),
            Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData),
            Directory.Exists(localLow) ? localLow : string.Empty
        }
        ...
        foreach (InstalledApplication application in inventory.Applications.OrderBy(item => item.DisplayName, StringComparer.OrdinalIgnoreCase))
        {
            ...
            if (cancellationToken.IsCancellationRequested)
            {
                ...
                issues.Add(new AppResidualDiscoveryIssue("search-roots", "Cancelled", "Discovery was cancelled by the caller."));
                break;
            }
            if (IsSystemOwned(application.DisplayName)) continue;
            ...
            foreach (string root in searchRoots)
            {
                ...
                string? path = TryGetDirectRootPath(root, application.DisplayName);
                ...
                candidates.Add(new AppResidualCandidate(
                    ...
                    "The uninstall registry still names this application, its recorded install location is missing, and an application data folder matches its display name.",
    ...
    private static readonly HashSet<string> SystemOwnedNames = new(StringComparer.OrdinalIgnoreCase)
    {
        "Microsoft",
        "Windows",
        "WindowsApps",
    ...
    private static bool IsSystemOwned(string? name)
    {
        if (string.IsNullOrWhiteSpace(name)) return true;
        string trimmed = name.Trim();
        if (trimmed.StartsWith('.')) return true;
        return SystemOwnedNames.Contains(trimmed);
    }
    ...
    private static string? TryGetDirectRootPath(string root, string displayName)
    {
        if (string.IsNullOrWhiteSpace(displayName) || displayName is "." or ".." || displayName.IndexOfAny(Path.GetInvalidFileNameChars()) >= 0 || IsSystemOwned(displayName)) return null;
        string candidate = Path.GetFullPath(Path.Combine(root, displayName));
