...
function Invoke-WinCareHeadlessCommand {
    ...
    $data=switch($Command){
        ...
        'peer-task-save'{$plan=New-WinCareTaskPlan -Id ([string](Get-WinCarePropertyValue $Arguments 'Id' '')) -Title ([string]$Arguments.Title) -Body ([string](Get-WinCarePropertyValue $Arguments 'Body' '')) -Status ([string](Get-WinCarePropertyValue $Arguments 'Status' 'Todo')) -Pinned:([bool](Get-WinCarePropertyValue $Arguments 'Pinned' $false));Invoke-WinCareHeadlessPlan -Plan $plan -Apply:$Apply}
