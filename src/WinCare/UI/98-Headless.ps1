...
function Get-WinCareHeadlessCommandName {
    ...
    ) + @(Get-WinCareAdvancedHeadlessCommandName)
}
...
function Invoke-WinCareHeadlessCommand {
    ...
    if($Command -in (Get-WinCareAdvancedHeadlessCommandName)){return Invoke-WinCareAdvancedHeadlessCommand -Command $Command -Arguments $Arguments -Apply:$Apply}
    $data=switch($Command){
        ...
        'note-save'{
            ...
            $links=if($existing){@($existing.LinkedOperationIds)}else{@()};$sources=if($existing){@($existing.SourceRecords)}else{@('SRC:66c214216c')}
