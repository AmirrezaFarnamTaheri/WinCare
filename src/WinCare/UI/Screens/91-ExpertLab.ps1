    ...
    while($true){
        ...
        try{
            switch($choice.Action){
                ...
                'Measure'{
                    $configuredHosts=@(Get-WinCareConfig 'NetworkExperimentTargets') -join ','
                    $hosts=(Read-WinCareInput -Prompt 'Comma-separated organization-controlled DNS hosts or IPs' -Default $configuredHosts).Split(',',[StringSplitOptions]::RemoveEmptyEntries)|ForEach-Object{$_.Trim()}
                    if(-not $hosts.Count){throw 'Enter at least one explicit endpoint or configure NetworkExperimentTargets.'}
                ...
                'Experiment'{
                    ...
                    $configuredHosts=@(Get-WinCareConfig 'NetworkExperimentTargets') -join ','
                    $hosts=(Read-WinCareInput -Prompt 'Comma-separated organization-controlled measurement hosts or IPs' -Default $configuredHosts).Split(',',[StringSplitOptions]::RemoveEmptyEntries)|ForEach-Object{$_.Trim()}
                    if(-not $hosts.Count){throw 'Enter at least one explicit endpoint or configure NetworkExperimentTargets.'}
