...
function Show-WinCarePeerUtilitiesScreen {
    while($true){
        $menu=@(
            [pscustomobject]@{Label='System toolkit convergence';Description='Diagnostics, hardening, maintenance templates, shortcut export, batch downloads, and torrent metadata';Action='Toolkit'},
            ...
            [pscustomobject]@{Label='Legacy Unsafe one-click profiles';Description='Permanent and destructive donor-derived profiles with Critical confirmation';Action='Unsafe'},
            [pscustomobject]@{Label='Cleanup and AppX convergence';Description='Explorer visibility, bounded deep cleanup, list/allowlist AppX removal, all-users and provisioned modes';Action='CleanupMods'},
        ...
        try{
            switch($choice.Action){
                'Toolkit'{
                    ...
                    $subChoice=Show-WinCareMenu -Title 'System toolkit convergence' -Subtitle 'Donor concepts are implemented through existing WinCare authority and state owners' -Items $sub
                ...
                'Experience'{
                    ...
                    $subChoice=Show-WinCareMenu -Title 'Experience Studio' -Subtitle 'Donor concepts are adapted to WinCare state, policy, transactions, and recovery' -Items $sub
                    if($subChoice -and $subChoice.Action -ne 'Back'){
                        switch($subChoice.Action){
                ...
                'CleanupMods'{
                    $sub=@(
                        ...
                        [pscustomobject]@{Label='Deep cleanup profiles';Description='Bounded safe or Critical donor-aggressive cleanup';Action='Deep'},
                    ...
                    $subChoice=Show-WinCareMenu -Title 'Cleanup and AppX convergence' -Subtitle 'All changes remain previewed and journaled; Critical profiles require explicit authorization' -Items $sub
                    if($subChoice -and $subChoice.Action -ne 'Back'){
                        switch($subChoice.Action){
