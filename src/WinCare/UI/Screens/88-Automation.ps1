...
function Show-WinCareAutomationScreen {
    while($true){$profiles=@(Get-WinCareAutomationProfile);$menu=@(
        ...
        'Create'{$id=Read-WinCareInput -Prompt 'Profile ID';$title=Read-WinCareInput -Prompt 'Title';$command=Read-WinCareInput -Prompt 'Headless command' -Default 'system';$schedule=Read-WinCareInput -Prompt 'Schedule (daily@HH:mm or weekly:Day@HH:mm)' -Default 'weekly:Sunday@03:00';$applyText=Read-WinCareInput -Prompt 'Apply modifying command? (y/N)' -Default 'N';$argsText=Read-WinCareInput -Prompt 'Arguments JSON' -Default '{}';try{$args=$argsText|ConvertFrom-Json -AsHashtable -Depth 20;$profile=[ordered]@{schemaVersion=1;id=$id;title=$title;command=$command;arguments=$args;apply=$applyText -match '^(?i)y';schedule=$schedule;maximumRuntimeMinutes=120;batteryAllowed=$false;networkRequired=$command -like 'wua-*';notification='Journal';sourceRecords=@('D08-MAINTENANCE-ORCHESTRATION','D09-DECLARATIVE-RESOURCES')};$path=Save-WinCareAutomationProfile -Profile $profile;Show-WinCareMessage -Title 'Profile saved' -Lines @($path) -Kind Success}catch{Show-WinCareMessage -Title 'Profile rejected' -Lines @($_.Exception.Message) -Kind Error}}
        ...
        'Folder'{Start-Process (Join-Path $script:WinCareState.Root 'Automation')}
    }}
}
