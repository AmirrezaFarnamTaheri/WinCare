...
function Show-WinCareWorkspaceStudioScreen {
    while($true){
        ...
        $choice=Show-WinCareMenu -Title 'Workspace and device studio' -Subtitle 'Target-native convergence of monitoring, file, layout, display, device, and workflow peers' -Items $menu
        ...
        try{
            switch($choice.Action){
                ...
                'Layouts'{
                    $profile=Show-WinCareListSelector -Title 'Workspace Studio layouts' -Subtitle 'Choose a target-native donor-derived layout' -Items @(Get-WinCareWorkspaceStudioProfile) -Formatter ${function:Format-WinCareWorkspaceStudioProfileRow} -SearchProperties @('Id','Name')
