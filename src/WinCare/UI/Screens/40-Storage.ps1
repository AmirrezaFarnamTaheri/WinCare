...
function Show-WinCareStorageScreen {
    while ($true) {
        ...
        switch ($choice.Action) {
            'Drives' { Show-WinCareDriveOverview }
            'Large' { Show-WinCareLargeFileScreen }
            'Sense' { Show-WinCareStorageSenseScreen }
            'Settings' {
                $null=Start-WinCareDetachedProcess -FilePath 'explorer.exe' `
                    -ArgumentList @('ms-settings:storage')
            }
        }
    }
}
