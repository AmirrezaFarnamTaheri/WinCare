...
function Get-WinCareGuiNavigation {
    ...
    if($Refresh -or $null -eq $script:WinCareGuiNavigationCache){
        ...
        $script:WinCareGuiNavigationCache=@(Get-Content -LiteralPath $path -Raw -Encoding utf8|ConvertFrom-Json -Depth 20)
    }
...
function Get-WinCareGuiActionCatalog {
    ...
    if($Refresh -or $null -eq $script:WinCareGuiActionCatalogCache){
        ...
        $curated=@(Get-Content -LiteralPath $path -Raw -Encoding utf8|ConvertFrom-Json -Depth 40)
