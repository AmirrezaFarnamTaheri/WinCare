...
function New-WinCareBcdExportPlan {
    ...
    $action=New-WinCareAction -Type BcdExport -Label 'Export the Boot Configuration Data store' -Risk Moderate -Parameters @{Destination=$destination} -RequiresAdmin $true -Reversible $false -Tags @('Boot','Backup') -SourceRecords @('SRC:283330b589') -RecoveryDescription 'The export is a backup artifact; WinCare does not automatically import it.'
    New-WinCarePlan -Title 'Export BCD backup' -Actions @($action) -RollbackOnFailure $false -SourceRecords @('SRC:283330b589')
}
