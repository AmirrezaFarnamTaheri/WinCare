...
function Get-WinCarePowerSessionRoot {
    Ensure-WinCareState
    Get-WinCareBridgeStateRoot 'PowerSessions'
}
...
function New-WinCarePowerSessionPlan {
    ...
    $action=New-WinCareAction -Type 'StartPowerSession' -Label "Start $Mode awake session" -Risk $(if($away){'High'}elseif($preventDisplay){'Moderate'}else{'Low'}) -Parameters @{Id=$id;Mode=$Mode;PreventDisplay=$preventDisplay;AwayMode=$away;ExpiresAt=$expires;RefreshSeconds=$RefreshSeconds;Reason=$Reason} -RequiresAdmin $false -Reversible $true -TimeoutSeconds 60 -Tags @('Power','Awake','Session') -SourceRecords @('SRC:f762344bb0') -RecoveryDescription 'Stop the exact WinCare power-session host and clear its execution-state request.'
    New-WinCarePlan -Title 'Start WinCare power session' -Description 'Runs a dedicated integrity-checked local host that renews the supported Windows execution-state request until expiry or explicit stop.' -Actions @($action) -SourceRecords @('SRC:f762344bb0')
}
...
function New-WinCarePowerSessionStopPlan {
    ...
    $action=New-WinCareAction -Type 'StopPowerSession' -Label "Stop $($record.Mode) awake session" -Risk Low -Parameters @{Id=$Id;ExpectedHostProcessId=[int]$record.HostProcessId;ExpectedHostProcessStartTime=[string]$record.HostProcessStartTime} -RequiresAdmin $false -Reversible $false -TimeoutSeconds 30 -Tags @('Power','Awake','Session') -SourceRecords @('SRC:f762344bb0') -RecoveryDescription 'A stopped awake session is not restarted automatically.'
    New-WinCarePlan -Title 'Stop WinCare power session' -Actions @($action) -RollbackOnFailure $false -SourceRecords @('SRC:f762344bb0')
}
