...
function Get-WinCareShellHardwareRoot {
    [CmdletBinding()]param()
    Ensure-WinCareState
    $root=Join-Path $script:WinCareState.Root 'ShellHardwareStudio'
...
function Get-WinCareHardwareInventory {
    ...
    if(-not $IsWindows){return [pscustomobject]@{Available=$false;Reason='WindowsRequired';SourceRecords=@('D149-NWINFO-HARDWARE-INVENTORY')}}
    ...
    [pscustomobject]@{
        ...
        Errors=@($errors);Bounds=[pscustomobject]@{MaximumItemsPerClass=$MaximumItemsPerClass};SourceRecords=@('D149-NWINFO-HARDWARE-INVENTORY','D155-DUPLICATE-SUBMISSION')
    }
}
...
function New-WinCareHardwareReportPlan {
    ...
    $action=New-WinCareManagedFileWriteAction -Path $path -Content $content -AllowedRoots @($root) -Label 'Write bounded hardware report' -SourceRecords @('D149-NWINFO-HARDWARE-INVENTORY')
...
function Get-WinCareWindowSearch {
    ...
    foreach($window in @(Get-WinCareWindowInventory -IncludeUntitled)){
        ...
        if($score -gt 0){$rows.Add([pscustomobject]@{Score=$score;Handle=$window.Handle;ProcessId=$window.ProcessId;ProcessName=$window.ProcessName;Title=$window.Title;Path=$window.Path;Signature=$window.Signature;SourceRecords=@('D148-WINDOW-WALKER-SEARCH','D163-EXPLORER-SESSION-RESTORE')})}
    }
    ...
    try{
        ...
        foreach($window in @($shell.Windows())){
            try{
                ...
                $rows.Add([pscustomobject]@{Path=$path;PathHash=Get-WinCareSha256Text -Text $path;LocationName=[string]$window.LocationName;Hwnd=[long]$window.HWND;SourceRecords=@('D144-7PLUS-EXPLORER-TABS','D163-EXPLORER-SESSION-RESTORE')})
...
function Get-WinCareExplorerSessionRoot {
    $root=Join-Path (Get-WinCareShellHardwareRoot) 'ExplorerSessions';if(-not(Test-Path -LiteralPath $root)){$null=New-Item -ItemType Directory -Path $root -Force};$root
}
...
function New-WinCareExplorerSessionSavePlan {
    ...
    $record=[ordered]@{SchemaVersion=1;Id=$Id;CapturedAt=[datetime]::UtcNow.ToString('o');Locations=$locations;SourceRecords=@('D144-7PLUS-EXPLORER-TABS','D163-EXPLORER-SESSION-RESTORE')}
...
function Get-WinCareExplorerSessionRecord {
    ...
    foreach($path in $paths){
        ...
        try{
            $record=Get-Content -LiteralPath $path -Raw -Encoding utf8|ConvertFrom-Json -AsHashtable -Depth 30
...
function New-WinCareExplorerSessionRestorePlan {
    ...
    foreach($location in @($record.Locations)){
        ...
        $actions.Add((New-WinCareAction -Type OpenExplorerLocation -Label "Open Explorer: $path" -Risk Low -Parameters @{Path=$path;ExpectedPathHash=[string]$location.PathHash} -RequiresAdmin $false -Reversible $false -TimeoutSeconds 60 -Tags @('Explorer','Session','Restore') -SourceRecords @('D144-7PLUS-EXPLORER-TABS','D163-EXPLORER-SESSION-RESTORE') -RecoveryDescription 'Opening an Explorer window is transient and is not automatically reversed.'))
    }
    New-WinCarePlan -Title "Restore Explorer session $Id" -Description 'Opens only canonical local directories that still match their captured path identities.' -Actions @($actions) -RollbackOnFailure $false -SourceRecords @('D144-7PLUS-EXPLORER-TABS','D163-EXPLORER-SESSION-RESTORE')
}
...
function Get-WinCareUiAutomationSnapshot {
    ...
    if(-not $IsWindows){return [pscustomobject]@{Available=$false;Nodes=@();SourceRecords=@('D154-GUI-INSPECT-BOUNDARY')}}
    ...
    [pscustomobject]@{Available=$true;ProcessId=$ProcessId;MaximumDepth=$MaximumDepth;MaximumNodes=$MaximumNodes;TextIncluded=[bool]$IncludeText;Truncated=($queue.Count -gt 0);NodeCount=$nodes.Count;Nodes=@($nodes);SourceRecords=@('D154-GUI-INSPECT-BOUNDARY','D147-WINDEV-XAML-AUDIT')}
}
    ...
    foreach($package in @(Get-AppxPackage -ErrorAction SilentlyContinue|Select-Object -First $Limit)){
        try{
            ...
            foreach($app in @($manifest.Package.Applications.Application)){
                ...
                $rows.Add([pscustomobject]@{Aumid=$target;ApplicationId=$id;PackageName=[string]$package.Name;PackageFullName=[string]$package.PackageFullName;PackageFamilyName=[string]$package.PackageFamilyName;ManifestPath=$manifestPath;ManifestSha256=Get-WinCareSha256 -LiteralPath $manifestPath;DisplayName=[string]$app.VisualElements.DisplayName;SourceRecords=@('D152-APPX-LAUNCH-METADATA')})
            }
...
function Get-WinCareServicingMediaInfo {
    ...
    [pscustomobject]@{Path=$path;Sha256=Get-WinCareSha256 -LiteralPath $path;Bytes=[long]$item.Length;Success=$result.Success;ExitCode=$result.ExitCode;Images=@($images);RawOutput=if($images.Count){''}else{[string]$result.Data.StdOut};ReadOnly=$true;SourceRecords=@('D158-BATUTIL-SERVICING-MEDIA')}
}
...
function Get-WinCareTerminalEnvironment {
    [CmdletBinding()]param()
    if(-not $IsWindows){return [pscustomobject]@{Available=$false;SourceRecords=@('D156-CONEMU-PROFILE-SESSIONS')}}
    $candidates=[Collections.Generic.List[string]]::new();$local=[Environment]::GetFolderPath('LocalApplicationData')
    if(-not $local){return [pscustomobject]@{Available=$false;Reason='LocalApplicationDataUnavailable';SourceRecords=@('D156-CONEMU-PROFILE-SESSIONS')}}
    ...
    [pscustomobject]@{Available=$true;WindowsTerminal=@($terminal);ConEmu=@($conEmu);SecretsIncluded=$false;Execution='None';SourceRecords=@('D156-CONEMU-PROFILE-SESSIONS','D162-TAB-MANAGER-COMMAND-PALETTE')}
}
...
function New-WinCareTerminalProfileExportPlan {
    ...
    $action=New-WinCareManagedFileWriteAction -Path $path -Content $content -AllowedRoots @($root) -Label 'Export sanitized terminal environment' -SourceRecords @('D156-CONEMU-PROFILE-SESSIONS')
...
function Get-WinCareGuiResourceAudit {
    ...
    $path=Assert-WinCareSafePath -LiteralPath $LiteralPath -AllowedRoots @($script:WinCareModuleRoot);$raw=Get-Content -LiteralPath $path -Raw -Encoding utf8
    if([Text.Encoding]::UTF8.GetByteCount($raw) -gt 2097152){throw 'XAML resource audit is limited to 2 MiB.'}
    ...
    [pscustomobject]@{Path=$path;Sha256=Get-WinCareSha256 -LiteralPath $path;DefinitionCount=$definitions.Count;ReferenceCount=$references.Count;Missing=@($missing);Unused=@($unused);Success=($missing.Count-eq 0);SourceRecords=@('D147-WINDEV-XAML-AUDIT','D143-V5-CAPABILITY-NAVIGATION')}
}
...
function Get-WinCareFullscreenShellAssessment {
    ...
    [pscustomobject]@{Shell=$shellValue;CustomShell=$custom;DeviceForm=$deviceFormValue;ExplorerRunning=[bool](Get-Process explorer -ErrorAction SilentlyContinue);Risk=if($custom){'Critical'}elseif($deviceFormCode -eq 46){'High'}else{'Low'};Admission='AssessmentOnly';Reason='Custom shell replacement is not admitted as an automatic WinCare mutation because startup failure can remove the primary recovery surface.';SourceRecords=@('D150-FULLSCREEN-SHELL-ASSESSMENT','D151-DUPLICATE-SUBMISSION')}
}

function Get-WinCareShellHardwareCapabilityCard {
    [CmdletBinding()]param([string]$Query='')
    $cards=@(
        [pscustomobject]@{Id='hardware-inventory';Title='Hardware inventory';Summary='Bounded CIM inventory with sensitive identifiers hashed by default.';Risk='ReadOnly';SourceRecords=@('D149-NWINFO-HARDWARE-INVENTORY')},
        [pscustomobject]@{Id='monitor-control';Title='DDC/CI monitor controls';Summary='Brightness and contrast with physical-monitor compare-and-swap and recovery.';Risk='Low';SourceRecords=@('D146-MONITORIAN-DDCCI')},
        [pscustomobject]@{Id='window-search';Title='Window and Explorer search';Summary='Fuzzy title/process search plus bounded Explorer session capture and restore.';Risk='Low';SourceRecords=@('D148-WINDOW-WALKER-SEARCH','D163-EXPLORER-SESSION-RESTORE')},
        [pscustomobject]@{Id='ui-automation';Title='UI Automation snapshot';Summary='Bounded accessibility-tree inspection with text redacted by default.';Risk='ReadOnly';SourceRecords=@('D154-GUI-INSPECT-BOUNDARY')},
        [pscustomobject]@{Id='appx-runtime';Title='AppX runtime';Summary='Process-package mapping and exact manifest-verified activation.';Risk='Low';SourceRecords=@('D152-APPX-PROCESS-PACKAGE-MAPPING','D152-APPX-LAUNCH-METADATA')},
        [pscustomobject]@{Id='archive-inspection';Title='Archive inspection';Summary='No-extraction ZIP/AppX/MSIX member and expansion safety audit.';Risk='ReadOnly';SourceRecords=@('D159-NANAZIP-ARCHIVE-SAFETY')},
        [pscustomobject]@{Id='servicing-media';Title='Servicing media';Summary='Read-only DISM metadata for local WIM, ESD, and SWM images.';Risk='ReadOnly';SourceRecords=@('D158-BATUTIL-SERVICING-MEDIA')},
        [pscustomobject]@{Id='terminal-environment';Title='Terminal environment';Summary='Sanitized Windows Terminal and ConEmu profile inventory and export.';Risk='ReadOnly';SourceRecords=@('D156-CONEMU-PROFILE-SESSIONS')},
        [pscustomobject]@{Id='fullscreen-shell';Title='Full-screen shell assessment';Summary='Detects custom shell replacement without granting mutation authority.';Risk='ReadOnly';SourceRecords=@('D150-FULLSCREEN-SHELL-ASSESSMENT')},
        [pscustomobject]@{Id='winhance-maximum';Title='Winhance maximum';Summary='Critical one-click hardening, debloat, privacy, and shell-reduction profile.';Risk='Critical';SourceRecords=@('D161-WINHANCE-MAXIMUM')}
    )
