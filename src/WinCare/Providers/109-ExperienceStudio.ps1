function Get-WinCareExperienceStudioRoot {
    [CmdletBinding()]param()
    Ensure-WinCareState
    Get-WinCareBridgeStateRoot 'ExperienceStudio'
}
...
function Get-WinCareVisualAssetInfo {
    ...
    [pscustomobject]@{
        ...
        MetadataOnly=$true;Execution='None';SourceRecords=@('SRC:5287f02b14')
    }
}
...
function Get-WinCareSpriteSheetLayout {
    ...
    [pscustomobject]@{SheetWidth=$SheetWidth;SheetHeight=$SheetHeight;FrameWidth=$FrameWidth;FrameHeight=$FrameHeight;Margin=$Margin;Spacing=$Spacing;Columns=$columns;Rows=$rows;Capacity=$capacity;FrameCount=$count;Frames=@($frames);SourceRecords=@('SRC:5287f02b14')}
}
...
function New-WinCareVisualAssetManifestPlan {
    ...
    $payload=[ordered]@{SchemaVersion=1;CreatedAt=[datetime]::UtcNow.ToString('o');Asset=$asset;SpriteLayout=$layout;SourceRecords=@('SRC:5287f02b14')}
    ...
    $action=New-WinCareManagedFileWriteAction -Path $path -Content $bytes -AllowedRoots @($root) -Label 'Write visual asset manifest' -SourceRecords @('SRC:5287f02b14')
...
function Get-WinCareLocationPrivacyState {
    ...
    [pscustomobject]@{EffectiveMode=$effective;MachinePolicy=$machine;UserConsent=$user;ServiceMutation='None';SourceRecords=@('SRC:5287f02b14')}
}
...
function Get-WinCareRemoteAccessProfile {
    ...
    $profiles=@(
        [pscustomobject]@{Id='rdp';Title='Remote Desktop';Description='RDP listener policy, TermService start/runtime state, firewall rules, and optional drive redirection.';Risk='High';SourceRecords=@('SRC:5287f02b14')},
        [pscustomobject]@{Id='openssh';Title='OpenSSH Server';Description='OpenSSH capability admission, service start/runtime state, and the exact server firewall rule.';Risk='High';SourceRecords=@('SRC:5287f02b14')}
    )
...
function Get-WinCareRemoteAccessState {
    ...
    [pscustomobject]@{RdpDeny=Get-WinCareRegistryValueState -Path 'HKLM:\SYSTEM\CurrentControlSet\Control\Terminal Server' -Name 'fDenyTSConnections';RdpDriveRedirection=Get-WinCareRegistryValueState -Path 'HKLM:\SOFTWARE\Policies\Microsoft\Windows NT\Terminal Services' -Name 'fDisableCdm';Services=@($services);FirewallRules=@($rules);OpenSshCapability=if($capability){[string]$capability.State}else{'Unavailable'};SourceRecords=@('SRC:5287f02b14')}
}
...
function Get-WinCarePowerConditionProfile {
    [CmdletBinding()]param([string]$Id='')
    $profiles=@(
        [pscustomobject]@{Id='battery-saver';Title='Battery saver';PowerScheme='a1841308-3541-4fab-bc81-f71556f20b4a';Brightness=35;Condition='Battery or charge below 35 percent';Risk='Low';SourceRecords=@('SRC:5287f02b14')},
        [pscustomobject]@{Id='balanced-mobile';Title='Balanced mobile';PowerScheme='381b4222-f694-41f0-9685-ff5bb260df2e';Brightness=65;Condition='Normal mobile operation';Risk='Low';SourceRecords=@('SRC:5287f02b14')},
        [pscustomobject]@{Id='performance-ac';Title='Performance on AC';PowerScheme='8c5e7fda-e8bf-4a96-9a85-a6e23a8c635c';Brightness=100;Condition='AC power and explicit performance preference';Risk='Moderate';SourceRecords=@('SRC:5287f02b14')}
    )
    if($Id){return @($profiles|Where-Object Id -eq $Id|Select-Object -First 1)}
    $profiles
}
...
function Get-WinCarePowerConditionAssessment {
    ...
    [pscustomobject]@{PowerLineStatus=$line;ChargePercent=$charge;BatteryPresent=[bool]$battery;RecommendedProfile=$recommended;Profiles=@(Get-WinCarePowerConditionProfile);SourceRecords=@('SRC:5287f02b14')}
}
...
function New-WinCarePowerConditionPlan {
    ...
    try{$brightness=New-WinCareBrightnessPlan -Percent ([int]$profile.Brightness);foreach($action in @($brightness.Actions)){$action.SourceRecords=@('SRC:5287f02b14');$actions.Add($action)}}catch{Write-Verbose 'Internal display brightness is unavailable; the power-scheme action remains valid.'}
...
function Get-WinCarePrivacyProfile {
    ...
    $profiles=@(
        [pscustomobject]@{Id='balanced';Title='Balanced privacy';Description='Reversible privacy settings without weakening Defender, SmartScreen, UAC, or Windows Update.';Risk='Moderate';RuleIds=$balanced;SourceRecords=@('SRC:5287f02b14')},
        [pscustomobject]@{Id='strict';Title='Strict privacy';Description='Adds consumer-feature, diagnostic-data, and cross-device restrictions.';Risk='High';RuleIds=$strict;SourceRecords=@('SRC:5287f02b14')},
        [pscustomobject]@{Id='maximum';Title='Maximum reversible privacy';Description='Largest reversible privacy set that remains inside WinCare catalog and recovery contracts.';Risk='High';RuleIds=$maximum;SourceRecords=@('SRC:5287f02b14')}
    )
    if($Id){return @($profiles|Where-Object Id -eq $Id|Select-Object -First 1)}
    $profiles
}
...
function Get-WinCareExperienceCapabilityCard {
    ...
    $cards=@(
        [pscustomobject]@{Id='visual-assets';Title='Visual assets and sprite sheets';Summary='Read-only image metadata, palette extraction, bounded frame maps, and protected manifest export.';Risk='Low';Commands=@('experience-visual-asset','experience-sprite-layout','experience-visual-manifest');SourceRecords=@('SRC:5287f02b14')},
        [pscustomobject]@{Id='location-privacy';Title='Location privacy';Summary='Machine policy and current-user consent without disabling the location service.';Risk='High';Commands=@('experience-location-state','experience-location-set');SourceRecords=@('SRC:5287f02b14')},
        [pscustomobject]@{Id='remote-access';Title='Remote access';Summary='RDP and OpenSSH state and reversible typed plans.';Risk='High';Commands=@('experience-remote-profiles','experience-remote-state','experience-remote-set');SourceRecords=@('SRC:5287f02b14')},
        [pscustomobject]@{Id='power-conditions';Title='Power condition profiles';Summary='Battery and AC-aware power-scheme and brightness recommendations.';Risk='Moderate';Commands=@('experience-power-profiles','experience-power-assess','experience-power-apply');SourceRecords=@('SRC:5287f02b14')},
        [pscustomobject]@{Id='download-strategy';Title='Download strategy';Summary='Explains WinCare BITS mirror failover, retry, scheduling, and integrity controls.';Risk='Low';Commands=@('experience-download-strategy');SourceRecords=@('SRC:5287f02b14')},
        [pscustomobject]@{Id='privacy-profiles';Title='Privacy profiles';Summary='Machine-readable reversible profiles plus separately authorized destructive presets.';Risk='High';Commands=@('experience-privacy-profiles','experience-privacy-apply','legacy-unsafe');SourceRecords=@('SRC:5287f02b14')}
    )
