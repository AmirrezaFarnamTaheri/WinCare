    ...
    foreach($log in $logs){
        try{
            ...
            foreach($event in @(Get-WinEvent -FilterHashtable $filter -MaxEvents $MaxEvents -ErrorAction Stop)){
                ...
                $result.Add([pscustomobject]@{TimeCreated=$event.TimeCreated;Id=$event.Id;Level=$event.LevelDisplayName;Provider=$event.ProviderName;Log=$event.LogName;Message=$message;SourceRecords=@('D34-BLUETOOTH-DEVICE-TELEMETRY')})
