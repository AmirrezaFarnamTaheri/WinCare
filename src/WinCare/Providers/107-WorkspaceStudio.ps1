function Get-WinCareWorkspaceStudioRoot {
    ...
    param()
    Ensure-WinCareState
    $root=Join-Path $script:WinCareState.Root 'WorkspaceStudio'
...
function New-WinCareFileWorkspacePlan {
    ...
    foreach($record in @($store.Records)){
        if([string]$record.Id -eq $Id){$records.Add([ordered]@{SchemaVersion=1;Id=$Id;Name=$Name;Paths=@($normalized|Sort-Object -Unique);Bookmarks=@($bookmarks|Sort-Object -Unique);SearchPattern=$SearchPattern;CreatedAt=[string]$record.CreatedAt;UpdatedAt=[datetime]::UtcNow.ToString('o');SourceRecords=@('D88-EXPLORER-TABS','D104-FILE-WORKSPACE')});$replaced=$true}else{$records.Add($record)}
    }
    if(-not $replaced){$records.Add([ordered]@{SchemaVersion=1;Id=$Id;Name=$Name;Paths=@($normalized|Sort-Object -Unique);Bookmarks=@($bookmarks|Sort-Object -Unique);SearchPattern=$SearchPattern;CreatedAt=[datetime]::UtcNow.ToString('o');UpdatedAt=[datetime]::UtcNow.ToString('o');SourceRecords=@('D88-EXPLORER-TABS','D104-FILE-WORKSPACE')})}
    if($records.Count -gt $limit){throw "File workspace catalog exceeds the configured limit of $limit."}
    New-WinCareWorkspaceStudioStorePlan -Kind FileWorkspaces -Records @($records) -SourceRecords @('D88-EXPLORER-TABS','D104-FILE-WORKSPACE')
}
...
function Get-WinCareWorkspaceStudioProfile {
    ...
    param([string]$Id='')
    $profiles=@(
        [pscustomobject]@{Id='columns-2';Name='Two equal columns';Ratios=@(@(0,0,0.5,1),@(0.5,0,0.5,1));SourceRecords=@('D88-EXPLORER-TABS','D89-UNITEDSETS-TABS')},
        [pscustomobject]@{Id='columns-3';Name='Three equal columns';Ratios=@(@(0,0,0.3333,1),@(0.3333,0,0.3334,1),@(0.6667,0,0.3333,1));SourceRecords=@('D89-UNITEDSETS-TABS','D101-NOTION-LAYOUTS')},
        [pscustomobject]@{Id='main-stack';Name='Main plus vertical stack';Ratios=@(@(0,0,0.67,1),@(0.67,0,0.33,0.5),@(0.67,0.5,0.33,0.5));SourceRecords=@('D101-NOTION-LAYOUTS','D100-FLOAT-AUTOALIGN')},
        [pscustomobject]@{Id='docked-side';Name='Docked side utility';Ratios=@(@(0,0,0.78,1),@(0.78,0,0.22,1));SourceRecords=@('D90-APPBAR-DOCK','D100-FLOAT-AUTOALIGN')},
        [pscustomobject]@{Id='rows-2';Name='Two equal rows';Ratios=@(@(0,0,1,0.5),@(0,0.5,1,0.5));SourceRecords=@('D101-NOTION-LAYOUTS')}
    )
...
function New-WinCareWorkspaceStudioLayoutPlan {
    ...
    $record=New-WinCareWorkspaceLayoutRecord -Name $Name -Description 'Workspace Studio donor-derived target-native layout.' -Slot @($slots)
...
function Get-WinCareUnifiedSystemSnapshot {
    ...
    [pscustomobject]@{SchemaVersion=1;CapturedAt=[datetime]::UtcNow.ToString('o');Telemetry=$telemetry;Processes=$processes;Services=$services;TcpConnections=$connections;SourceRecords=@('D87-WINDOWS-MONITORING','D92-SYSTEM-EXPLORER','D95-TASKBAR-MONITOR')}
}
...
'@
    ...
    $actions=@(
        (New-WinCareManagedFileWriteAction -Path (Join-Path $root "$Name-telegraf.conf") -Content ([Text.Encoding]::UTF8.GetBytes($telegraf)) -AllowedRoots @($root) -Label 'Write local-only Telegraf input profile' -SourceRecords @('D87-WINDOWS-MONITORING')),
        (New-WinCareManagedFileWriteAction -Path (Join-Path $root "$Name-grafana.json") -Content ([Text.Encoding]::UTF8.GetBytes((ConvertTo-WinCareCanonicalJson -InputObject $dashboard -Depth 30)+"`n")) -AllowedRoots @($root) -Label 'Write Grafana dashboard template' -SourceRecords @('D87-WINDOWS-MONITORING','D95-TASKBAR-MONITOR'))
    )
    New-WinCarePlan -Title 'Export local monitoring templates' -Description 'Creates bounded local input and dashboard templates without credentials, listeners, or network destinations.' -Actions $actions -SourceRecords @('D87-WINDOWS-MONITORING','D95-TASKBAR-MONITOR')
}
...
function Get-WinCareSyncthingState {
    ...
    if(-not $ConfigPath){
        if([string]::IsNullOrWhiteSpace($env:LOCALAPPDATA)){return [pscustomobject]@{ConfigPath='';ConfigPresent=$false;Processes=@();ApiKeyRedacted=$true;Reason='LOCALAPPDATA is unavailable.';SourceRecords=@('D99-SYNCTRAYZOR-LIFECYCLE')}}
    ...
    if(-not(Test-Path -LiteralPath $ConfigPath -PathType Leaf)){return [pscustomobject]@{ConfigPath=$ConfigPath;ConfigPresent=$false;Processes=$processes;ApiKeyRedacted=$true;SourceRecords=@('D99-SYNCTRAYZOR-LIFECYCLE')}}
    ...
    [pscustomobject]@{ConfigPath=$ConfigPath;ConfigPresent=$true;Sha256=Get-WinCareSha256 -LiteralPath $ConfigPath;Folders=$folders;Devices=$devices;Processes=$processes;GuiAddress=[string]$xml.configuration.gui.address;ApiKeyRedacted=$true;SourceRecords=@('D99-SYNCTRAYZOR-LIFECYCLE')}
}
...
return M
'@
    $action=New-WinCareManagedFileWriteAction -Path (Join-Path $root $Name) -Content ([Text.Encoding]::UTF8.GetBytes($lua)) -AllowedRoots @($root) -Label 'Write local WezTerm status module' -SourceRecords @('D98-WEZTERM-STATUS-BAR')
    New-WinCarePlan -Title 'Create WezTerm status module' -Description 'Creates an offline local module for workspace, host, current directory, and time status.' -Actions @($action) -SourceRecords @('D98-WEZTERM-STATUS-BAR')
}
...
function Get-WinCarePlaybookGraph {
    ...
    [pscustomobject]@{SchemaVersion=1;PlanId=[string]$Plan.Id;Title=[string]$Plan.Title;Nodes=@($nodes);Edges=@($edges);StopOnFailure=[bool]$Plan.StopOnFailure;RollbackOnFailure=[bool]$Plan.RollbackOnFailure;SourceRecords=@('D97-COMET-CONTROL-CONCEPTS','D105-FLOGO-ACTION-GRAPH')}
}
...
function New-WinCareBrightnessSchedulePlan {
    ...
    foreach($record in @($store.Records)){if([string]$record.Id -eq $Id){$records.Add([ordered]@{SchemaVersion=1;Id=$Id;Name=$Name;Steps=@($steps|Sort-Object Time);CreatedAt=[string]$record.CreatedAt;UpdatedAt=[datetime]::UtcNow.ToString('o');SourceRecords=@('D91-TWINKLE-SCHEDULES')});$replaced=$true}else{$records.Add($record)}}
    if(-not $replaced){$records.Add([ordered]@{SchemaVersion=1;Id=$Id;Name=$Name;Steps=@($steps|Sort-Object Time);CreatedAt=[datetime]::UtcNow.ToString('o');UpdatedAt=[datetime]::UtcNow.ToString('o');SourceRecords=@('D91-TWINKLE-SCHEDULES')})}
    ...
    New-WinCareWorkspaceStudioStorePlan -Kind BrightnessSchedules -Records @($records) -SourceRecords @('D91-TWINKLE-SCHEDULES')
}
...
function New-WinCareBrightnessScheduleApplyPlan {
    ...
    $plan=New-WinCareBrightnessPlan -Percent ([int]$selected[0].Percent);$plan.Title="Apply brightness schedule: $($schedule.Name)";$plan.SourceRecords=@('D91-TWINKLE-SCHEDULES');foreach($action in $plan.Actions){$action.SourceRecords=@('D91-TWINKLE-SCHEDULES','D10-QUICK-CONTROLS')};$plan
}
...
function Get-WinCareFolderAppearanceState {
    ...
    $payload=[ordered]@{SchemaVersion=1;Path=$folder.FullName;FolderAttributes=[int]$folder.Attributes;DesktopIniExists=$exists;DesktopIniContentBase64=if($exists){[Convert]::ToBase64String([IO.File]::ReadAllBytes($ini))}else{''};DesktopIniAttributes=if($exists){[int](Get-Item -LiteralPath $ini -Force).Attributes}else{0}}
...
function New-WinCareVerifiedPeerToolPlan {
    ...
    $record=if($ToolId -eq 'ViVeTool'){'D102-XBOX-FSE-FEATURES'}else{'D93-ADB-VERIFIED-TOOL'}
...
function New-WinCareXboxFullscreenExperiencePlan {
    ...
    $registry=New-WinCareAction -Type SetRegistryValue -Label $(if($Enabled){'Set Xbox full-screen device form'}else{'Clear Xbox full-screen device form marker'}) -Risk Critical -Parameters @{Path=$path;Name=$name;Value=if($Enabled){46}else{0};ValueType='DWord';Before=$before} -RequiresAdmin $true -Reversible $true -Compensator $registryComp -Tags @('Xbox','LegacyUnsafe','DeviceForm') -SourceRecords @('D102-XBOX-FSE-DEVICEFORM') -RecoveryDescription 'Restore the exact prior DeviceForm registry value.' -RestartPossible $true
    $tool=New-WinCareAction -Type RunVerifiedPeerTool -Label "$(if($Enabled){'Enable'}else{'Disable'}) Xbox Full Screen Experience feature IDs" -Risk Critical -Parameters @{ToolId='ViVeTool';FilePath=$verifiedPath;Sha256=$Sha256.ToLowerInvariant();Arguments=@($verb,$ids);Mutation=$true;SuccessExitCodes=@(0)} -RequiresAdmin $true -Reversible $true -Compensator @{Type='RunVerifiedPeerTool';RequiresAdmin=$true;Parameters=@{ToolId='ViVeTool';FilePath=$verifiedPath;Sha256=$Sha256.ToLowerInvariant();Arguments=@($opposite,$ids);Mutation=$true;SuccessExitCodes=@(0)}} -TimeoutSeconds 120 -Tags @('Xbox','LegacyUnsafe','ViVeTool') -SourceRecords @('D102-XBOX-FSE-FEATURES') -RecoveryDescription 'Run the exact opposite feature toggle with the same verified ViVeTool bytes.' -RestartPossible $true
    New-WinCarePlan -Title "$(if($Enabled){'Enable'}else{'Disable'}) Xbox Full Screen Experience" -Description 'Applies the donor-derived hidden Windows feature IDs and DeviceForm mutation. This can destabilize the shell and requires a reboot.' -Actions @($registry,$tool) -RollbackOnFailure $true -Metadata @{LegacyUnsafeProfile='xbox-fullscreen-experience';Acknowledgement='I ACCEPT LEGACY UNSAFE MUTATIONS';XboxFeatureIds=$ids;RebootRequired=$true} -SourceRecords @('D102-XBOX-FSE-DEVICEFORM','D102-XBOX-FSE-FEATURES')
}
