function Initialize-WinCareCommandLineInterop {
    ...
    if ('WinCare.Native.CommandLine' -as [type]) { return }
...
function Split-WinCareCommandLine {
    ...
    if ($IsWindows) {
        Initialize-WinCareCommandLineInterop
        try { return [WinCare.Native.CommandLine]::Split($CommandLine) } catch { Write-Verbose 'A best-effort operation was unavailable.' }
    }
...
function Get-WinCareInstalledApplication {
    ...
    Ensure-WinCareState
    $cacheKey = "apps:$([bool]$IncludeSystem)"
    if (-not $Refresh -and $script:WinCareState.ContainsKey($cacheKey)) {
        ...
        if (Get-WinCareConfig 'IncludeStoreApps') { $all += @(Get-WinCareAppxApplication -IncludeSystem:$IncludeSystem) }
        $dedup = @{}
        foreach ($app in $all) {
    ...
    if ($Search) {
        return @($apps | Where-Object { $_.Name -like "*$Search*" -or $_.Publisher -like "*$Search*" })
    }
    return @($apps)
}
...
function Invoke-WinCareRepairAction {
    param([object]$Action)
    Invoke-WinCareProcess -FilePath $Action.Parameters.FilePath -ArgumentList @($Action.Parameters.Arguments) -RequireAdmin:$Action.RequiresAdmin -TimeoutSeconds 7200 -SuccessExitCodes @($Action.Parameters.SuccessExitCodes)
}
