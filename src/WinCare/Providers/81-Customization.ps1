...
function Resolve-WinCareRequiredExecutable {
    ...
    foreach($candidate in @($candidates)){
        if($candidate.Contains('*')){foreach($item in @(Get-ChildItem -Path $candidate -File -ErrorAction SilentlyContinue|Sort-Object FullName -Descending)){return $item.FullName}}
        elseif($candidate -and (Test-Path -LiteralPath $candidate -PathType Leaf)){return $candidate}
    }
    return $null
}
...
function Get-WinCareContextMenuRuleState {
    ...
    foreach($rule in @($rules)){
        ...
        foreach($tree in @($rule.trees)){
            ...
            $expected=@{}
            foreach($value in @($tree.values)){$expected[[string]$value.name]=$value}
...
function New-WinCareContextMenuPlan {
    ...
    foreach($tree in $trees){
        ...
        if($Enable){
            $values=@($tree.values|ForEach-Object{@{Name=[string]$_.name;Value=$_.value;ValueType=[string]$_.valueType}})
...
function Get-WinCareCustomizationHost {
    [CmdletBinding()]param()
    $known=@(
        @{Id='windhawk';Name='Windhawk';Processes=@('windhawk');Paths=@((Join-Path $env:LOCALAPPDATA 'Programs\Windhawk'),(Join-Path $env:ProgramFiles 'Windhawk'));SourceRecords=@('D28-MOD-HOST')},
        @{Id='explorerpatcher';Name='ExplorerPatcher';Processes=@('ep_setup','ExplorerPatcher');Paths=@((Join-Path $env:ProgramFiles 'ExplorerPatcher'),(Join-Path $env:LOCALAPPDATA 'ExplorerPatcher'));SourceRecords=@('D31-EXPLORER-CUSTOMIZER')},
        @{Id='rainmeter';Name='Rainmeter';Processes=@('Rainmeter');Paths=@((Join-Path $env:ProgramFiles 'Rainmeter'),(Join-Path ${env:ProgramFiles(x86)} 'Rainmeter'));SourceRecords=@('D22-WIDGET-RUNTIME')},
        @{Id='contextmenuforwindows11';Name='ContextMenuForWindows11';Processes=@('ContextMenuCustom');Paths=@();AppxNames=@('f830d6cf-c83d-48b1-a9f6-95927d5f5b1c','852637cb-5c5e-4695-93cd-d2b0ca79f6de','31896935-DD54-4362-ACDD-6DA496624671','7061touchwp.CustomContextMenu');SourceRecords=@('D29-MODERN-MENU-HOST')},
        @{Id='airpodsdesktop';Name='AirPods Desktop';Processes=@('AirPodsDesktop');Paths=@((Join-Path $env:LOCALAPPDATA 'Programs\AirPodsDesktop'));SourceRecords=@('D34-BLUETOOTH-UX')}
    )
...
function Get-WinCareRainmeterSkinInventory {
    ...
    foreach($ini in @(Get-ChildItem -LiteralPath $root -Filter '*.ini' -File -Recurse -ErrorAction SilentlyContinue|Where-Object{($_.Attributes -band [IO.FileAttributes]::ReparsePoint)-eq 0 -and $_.Length -le 1048576}|Sort-Object FullName|Select-Object -First 4096)){
        $relative=[IO.Path]::GetRelativePath($root,$ini.FullName).Replace('\','/');$content=Get-Content -LiteralPath $ini.FullName -Raw -ErrorAction SilentlyContinue
        ...
        [pscustomobject]@{Path=$ini.FullName;RelativePath=$relative;Sha256=Get-WinCareSha256 $ini.FullName;Size=$ini.Length;Sections=$sections;Measures=$measures;Meters=$meters;SourceRecord='D22-SKIN-MEASURE-METER'}
    }
}
...
function Get-WinCareWindhawkModInventory {
    ...
    foreach($root in $roots){
        foreach($file in @(Get-ChildItem -LiteralPath $root -Filter '*.wh.cpp' -File -ErrorAction SilentlyContinue|Where-Object{($_.Attributes -band [IO.FileAttributes]::ReparsePoint)-eq 0 -and $_.Length -le 2097152}|Sort-Object Name|Select-Object -First 4096)){
            ...
            [pscustomobject]@{Id=[IO.Path]::GetFileNameWithoutExtension($file.Name);Path=$file.FullName;Sha256=Get-WinCareSha256 $file.FullName;Name=[string](Get-WinCarePropertyValue $meta 'name' $file.BaseName);Version=[string](Get-WinCarePropertyValue $meta 'version' '');Author=[string](Get-WinCarePropertyValue $meta 'author' '');Description=[string](Get-WinCarePropertyValue $meta 'description' '');SourceRecord='D28-MOD-MANIFEST'}
        }
    }
}
...
function Get-WinCareExplorerPatcherState {
    ...
    [pscustomobject]@{Installed=[bool]$snapshot.Exists;RegistryPath=$path;SnapshotHash=Get-WinCareRegistryTreeSnapshotHash $snapshot;Snapshot=$snapshot;SourceRecords=@('D31-SETTINGS-MODEL','D38-START-MENU-STYLING')}
}
