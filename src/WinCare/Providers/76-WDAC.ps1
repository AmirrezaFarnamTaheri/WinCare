...
function New-WinCareWdacDeployPlan {
    ...
    $action=New-WinCareAction -Type DeployWdacPolicy -Label "Deploy WDAC policy $($analysis.PolicyId)" -Risk $risk -Parameters @{Path=$analysis.Path;PolicyId=$analysis.PolicyId;Enforced=$enforced;Sha256=$analysis.Sha256} -RequiresAdmin $true -Reversible $true -Compensator @{Type='RemoveWdacPolicy';RequiresAdmin=$true;Parameters=@{PolicyId=$analysis.PolicyId}} -TimeoutSeconds 300 -Tags @('WDAC','CodeIntegrity') -SourceRecords @('D04-WDAC-DEPLOYMENT') -RecoveryDescription 'Remove the deployed policy with CiTool; a restart may be required.' -RestartPossible $true
    New-WinCarePlan -Title 'Deploy Windows Defender Application Control policy' -Actions @($action) -SourceRecords @('D04-WDAC-DEPLOYMENT')
}
