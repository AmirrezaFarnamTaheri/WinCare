...
function New-WinCareWidgetSnapshotExportPlan {
    ...
    $plan=New-WinCarePlan -Title 'Export widget snapshot' -Description 'Write a bounded, local dashboard snapshot using exact-content file semantics.' -SourceRecords @('D22-WIDGET-RUNTIME','D25-MAINTENANCE-METRICS','D34-BLUETOOTH-DEVICE-TELEMETRY','D36-DASHBOARD-CARDS') -Metadata @{Path=$full}
    $null=Add-WinCarePlanAction -Plan $plan -Action (New-WinCareManagedFileWriteAction -Path $full -Content $content -AllowedRoots @($root) -Label 'Write widget snapshot' -SourceRecords @('D22-WIDGET-RUNTIME','D25-MAINTENANCE-METRICS','D34-BLUETOOTH-DEVICE-TELEMETRY','D36-DASHBOARD-CARDS'))
    return $plan
}
