...
function Get-WinCareWindowZoneDefinition {
    ...
    $doc=Get-Content -LiteralPath $path -Raw|ConvertFrom-Json -AsHashtable -Depth 20
...
function New-WinCareWindowZonePlan {
    ...
    $action=New-WinCareAction -Type 'SetWindowBounds' -Label "Place $($window.ProcessName) in $($zone.LayoutTitle) / $($zone.ZoneTitle)" -Risk Low -Parameters $parameters -RequiresAdmin $false -Reversible $true -Compensator @{Type='SetWindowBounds';Parameters=$before} -TimeoutSeconds 30 -Tags @('Window','Zone','Idempotent') -SourceRecords @('W3-ZONES-DPI-RATIOS','W3-WINDOW-IDENTITY-CAS') -RecoveryDescription 'Restore the exact previous window rectangle while the same process and window remain alive.'
    New-WinCarePlan -Title 'Apply window zone' -Actions @($action) -SourceRecords @('W3-ZONES-DPI-RATIOS','W3-WINDOW-IDENTITY-CAS')
}
...
function New-WinCareWindowTopmostPlan {
    ...
    $action=New-WinCareAction -Type 'SetWindowTopmost' -Label "Set always-on-top to $Topmost for $($window.ProcessName)" -Risk Low -Parameters $parameters -RequiresAdmin $false -Reversible $true -Compensator @{Type='SetWindowTopmost';Parameters=$before} -TimeoutSeconds 30 -Tags @('Window','Topmost','Idempotent') -SourceRecords @('W3-ALWAYS-ON-TOP','W3-WINDOW-IDENTITY-CAS') -RecoveryDescription 'Restore the exact previous topmost state while the same window remains alive.'
    New-WinCarePlan -Title 'Change always-on-top state' -Actions @($action) -SourceRecords @('W3-ALWAYS-ON-TOP','W3-WINDOW-IDENTITY-CAS')
}

function New-WinCareWindowActivatePlan {
    [CmdletBinding()]param([Parameter(Mandatory)][long]$WindowHandle)
    $window=Get-WinCareWindowInventory -IncludeUntitled|Where-Object Handle -eq $WindowHandle|Select-Object -First 1;if(-not $window){throw 'Window was not found.'};if(-not $window.ProcessStartTime -or -not $window.ProcessName){throw 'Window process identity could not be established.'}
    $action=New-WinCareAction -Type 'ActivateWindow' -Label "Activate $($window.ProcessName): $($window.Title)" -Risk Low -Parameters @{WindowHandle=[long]$window.Handle;ProcessId=[int]$window.ProcessId;ProcessStartTime=[string]$window.ProcessStartTime;ExpectedTitleHash=[string]$window.TitleHash} -RequiresAdmin $false -Reversible $false -TimeoutSeconds 15 -Tags @('Window','Activation') -SourceRecords @('W3-WINDOW-MRU-FILTER') -RecoveryDescription 'Focus changes are transient and are not automatically reversed.'
    New-WinCarePlan -Title 'Activate window' -Actions @($action) -RollbackOnFailure $false -SourceRecords @('W3-WINDOW-MRU-FILTER')
}

function New-WinCareEmergencyInputReleasePlan {
    [CmdletBinding()]param()
    $action=New-WinCareAction -Type 'ReleaseLocalInput' -Label 'Release cursor clipping and pressed modifier state' -Risk Low -Parameters @{} -RequiresAdmin $false -Reversible $false -TimeoutSeconds 15 -Tags @('Input','EmergencyRelease') -SourceRecords @('W3-REMOTE-EMERGENCY-RELEASE','W3-EXPLICIT-CONTROL-HANDOFF') -RecoveryDescription 'The action only releases local input constraints; no persistent state is changed.'
    New-WinCarePlan -Title 'Emergency local input release' -Actions @($action) -RollbackOnFailure $false -SourceRecords @('W3-REMOTE-EMERGENCY-RELEASE','W3-EXPLICIT-CONTROL-HANDOFF')
}
