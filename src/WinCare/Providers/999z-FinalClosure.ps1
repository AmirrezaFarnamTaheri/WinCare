...
${function:Get-WinCareTcpGlobalState} = {
    [CmdletBinding()]param()
    $state=[ordered]@{Supported=$false;AutoTuningLevel='Unknown';EcnCapability='Unknown';ReceiveSideScaling='Unknown';ReceiveSegmentCoalescing='Unknown';Raw=@();CapturedAt=[datetime]::UtcNow.ToString('o')}
...
${function:Test-WinCareInjectionSurfaceSnapshot} = {
    ...
    $exact=@('HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Windows','HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows NT\CurrentVersion\Windows','HKLM:\SYSTEM\CurrentControlSet\Control\Session Manager\AppCertDlls')
    ...
    if($map.Contains('Values')){
        foreach($value in @($map.Values)){
            ...
            if([string](Get-WinCarePropertyValue $entry 'Path' '') -ne $path -or [string](Get-WinCarePropertyValue $entry 'Name' '') -ne $name){throw 'Injection-surface recovery values do not match the declared target.'}
        }
...
${function:New-WinCareWorkspaceLayoutRecord} = {
    ...
    $slots=@($Slot|ForEach-Object{
        ...
        [ordered]@{Id=$slotId;ProcessName=[string](Get-WinCarePropertyValue $map 'ProcessName' '');TitlePattern=[string](Get-WinCarePropertyValue $map 'TitlePattern' '');MonitorDevice=[string](Get-WinCarePropertyValue $map 'MonitorDevice' '');LeftRatio=[double](Get-WinCarePropertyValue $map 'LeftRatio' 0);TopRatio=[double](Get-WinCarePropertyValue $map 'TopRatio' 0);WidthRatio=[double](Get-WinCarePropertyValue $map 'WidthRatio' 0);HeightRatio=[double](Get-WinCarePropertyValue $map 'HeightRatio' 0);Required=[bool](Get-WinCarePropertyValue $map 'Required' $true)}
    })
...
${function:Test-WinCarePlaybookDefinition} = {
    ...
    $null=Test-WinCareStrictObjectKeys -InputObject $p -AllowedKeys @('id','title','description','minimumBuild','maximumBuild','architectures','requiresAdmin','sourceRecords','steps') -Context 'playbook'
    ...
    foreach($stepValue in $steps){
        ...
        switch($kind){
            'preset'{$null=Test-WinCareStrictObjectKeys $step @('kind','id') 'playbook preset step';if([string]$step.id -notmatch '^[a-z0-9][a-z0-9._-]{2,80}$'){throw 'Playbook preset ID is invalid.'}}
            ...
            'context-menu'{$null=Test-WinCareStrictObjectKeys $step @('kind','id','enable') 'playbook context-menu step';if($step.enable -isnot [bool]){throw 'Playbook context-menu enable must be Boolean.'}}
            'app-removal'{$null=Test-WinCareStrictObjectKeys $step @('kind','id','includeProvisioned') 'playbook app-removal step';if($step.includeProvisioned -isnot [bool]){throw 'Playbook app-removal flag must be Boolean.'}}
        }
    }
    $true
}
${function:Get-WinCarePlaybook} = {
    ...
    foreach($playbook in @($document.playbooks)){$null=Test-WinCarePlaybookDefinition $playbook;if(-not $seen.Add([string]$playbook.id)){throw 'Duplicate playbook ID.'}}
    if($Id){return @($document.playbooks|Where-Object id -eq $Id)}
    @($document.playbooks)
}
...
${function:Get-WinCareLegacyUnsafeProfile} = {
    ...
    if(-not @($profiles|Where-Object Id -eq 'optimize-debloat-ultimate').Count){$profiles.Add((New-WinCareLegacyProfileRecord -Id 'optimize-debloat-ultimate' -Title 'Ultimate cleanup compatibility profile' -Description 'Routes aggressive donor cleanup intent to the bounded reviewed deep-clean workflow.' -PlanKind DeepCleanup -TargetId 'windows-cleaner-utility-all' -SourceRecords @('SRC:6e8ae24ac4')))}
    if(-not @($profiles|Where-Object Id -eq 'personalization-legacy').Count){$profiles.Add((New-WinCareLegacyProfileRecord -Id 'personalization-legacy' -Title 'Legacy personalization compatibility profile' -Description 'Routes legacy personalization intent to reviewed reversible appearance controls.' -Controls @('AdvertisingId','TailoredExperiences','ConsumerFeatures') -SourceRecords @('SRC:1c7d9e2add')))}
...
${function:New-WinCareProvisioningPlan} = {
    ...
    $plan=New-WinCarePlan -Title ("{0} [{1}]" -f [string]$Blueprint.name,$Stage) -Description ([string]$Blueprint.description) -SourceRecords $sources
    ...
    if($Stage -in @('All','System')){
        ...
        foreach($item in @($Blueprint.optionalFeatures)){$state=Test-WinCareProvisioningStateItem $item Feature;$plan.Actions.Add((New-WinCareAction -Type SetOptionalFeatureState -Label "$($state.State) optional feature $($state.Name)" -Risk Moderate -Parameters @{Name=$state.Name;State=$state.State} -RequiresAdmin $true -Reversible $false -RestartPossible $true -Tags @('Provisioning','Feature','SystemStage') -SourceRecords @('SRC:4e4b1fa0d3')))}
        foreach($item in @($Blueprint.capabilities)){$state=Test-WinCareProvisioningStateItem $item Capability;$plan.Actions.Add((New-WinCareAction -Type SetCapabilityState -Label "$($state.State) capability $($state.Name)" -Risk Moderate -Parameters @{Name=$state.Name;State=$state.State} -RequiresAdmin $true -Reversible $false -RestartPossible $true -Tags @('Provisioning','Capability','SystemStage') -SourceRecords @('SRC:4e4b1fa0d3')))}
    }
    if($Stage -in @('All','User')){
        Add-WinCareProvisioningCatalogRules $plan @($Blueprint.userStage) 'Provisioning user-stage configuration'
        foreach($name in @($Blueprint.removeInstalledAppx)){$plan.Actions.Add((New-WinCareAction -Type RemoveAppxPackage -Label "Remove installed package $name" -Risk Moderate -Parameters @{PackageFullName=[string]$name;Name=[string]$name} -Tags @('Provisioning','Appx','UserStage') -SourceRecords @('SRC:4e4b1fa0d3') -RecoveryDescription 'Reinstall from the Microsoft Store or installation media where available.'))}
        foreach($id in @($Blueprint.wingetPackages)){$plan.Actions.Add((New-WinCareAction -Type InstallWingetPackage -Label "Install WinGet package $id" -Risk Moderate -Parameters @{PackageId=[string]$id} -TimeoutSeconds 7200 -Tags @('Provisioning','WinGet','UserStage') -SourceRecords @('SRC:4e4b1fa0d3')))}
    }
    $validation=Test-WinCarePlanContract $plan;if(-not $validation.Success){throw "Provisioning plan is invalid: $($validation.Message)"}
    $plan
}
