function Get-WinCareDownloadRoot {
    Ensure-WinCareState
    $root=Join-Path $script:WinCareState.Root 'Downloads'
    foreach($name in @('','Partial','Completed','Exports')){$path=if($name){Join-Path $root $name}else{$root};$null=New-Item -ItemType Directory -Path $path -Force}
    $root
}
...
function Test-WinCareDownloadUri {
    ...
    if($parsed.Scheme -eq 'http' -and -not [bool](Get-WinCarePolicy 'AllowInsecureDownloads')){throw 'Insecure HTTP downloads are disabled by policy.'}
...
function New-WinCareDownloadJobRecord {
    ...
    param(
        ...
        [string[]]$SourceRecords=@('D54-DOWNLOAD-QUEUE','D55-DOWNLOAD-SCHEDULER','D65-TRANSFER-STATE','D66-DOWNLOAD-UI','D69-ASYNC-RESUME')
    )
    Ensure-WinCareState
...
function Invoke-WinCareStartDownloadTransferAction {
    ...
    $source=[string]$record.Urls[$sourceIndex];$temp=Assert-WinCareSafePath -LiteralPath ([string]$record.TemporaryPath) -AllowedRoots @((Join-Path (Get-WinCareDownloadRoot) 'Partial')) -AllowMissing
    ...
    try{
        ...
        if([long]$bits.BytesTotal -gt (Get-WinCareDownloadByteLimit)){try{Remove-BitsTransfer -BitsJob $bits -Confirm:$false -ErrorAction Stop}catch{Write-WinCareLog -Level Warning -Message 'Oversized BITS job could not be removed immediately.' -Data @{error=$_.Exception.Message}};throw 'Download size exceeds the policy byte limit.'}
        ...
        New-WinCareResult -Success $true -Code 'DownloadTransferStarted' -Message 'BITS transfer started and bound to the protected queue record.' -Data @{Job=$saved;Compensator=$compensator}
    }catch{
...
function Invoke-WinCareReconcileDownloadTransferAction {
    ...
    try{
        $bits=Get-BitsTransfer -Id ([guid]$record.BitsJobId) -ErrorAction Stop;$copy.BytesTotal=[long]$bits.BytesTotal;$copy.BytesTransferred=[long]$bits.BytesTransferred;if([long]$bits.BytesTotal -gt (Get-WinCareDownloadByteLimit)){try{Remove-BitsTransfer -BitsJob $bits -Confirm:$false -ErrorAction Stop}catch{Write-WinCareLog -Level Warning -Message 'Oversized BITS job could not be removed during reconciliation.' -Data @{error=$_.Exception.Message}};throw 'Download size exceeds the policy byte limit.'}
        ...
        $next=Set-WinCareDownloadRecordInStore $store $copy;$null=Set-WinCareDownloadStoreInternal $next
        New-WinCareResult -Success $true -Code 'DownloadReconciled' -Message "Download reconciled to $($copy.Status)." -Data $copy
    }catch{
        ...
        New-WinCareResult -Success $false -Code 'DownloadReconcileFailed' -Message $copy.LastError -ExitCode 1 -Data $copy
    }
}
