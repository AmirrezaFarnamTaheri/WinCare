...
function New-WinCareOfflineDriverAddPlan {
    ...
    $action=New-WinCareAction -Type OfflineImageAddDriver -Label "Add driver to offline image: $([IO.Path]::GetFileName($driver))" -Risk $(if($ForceUnsigned){'Critical'}else{'High'}) -Parameters @{ImagePath=$image;DriverPath=$driver;Recurse=[bool]$Recurse;ForceUnsigned=[bool]$ForceUnsigned} -RequiresAdmin $true -Reversible $true -Tags @('OfflineImage','Driver','Servicing') -SourceRecords @('SRC:d9c8da5094') -RecoveryDescription 'Remove newly added OEM driver packages detected by before/after inventory.' -RestartPossible $false -TimeoutSeconds 21600
    New-WinCarePlan -Title 'Add driver to offline image' -Description 'Service a mounted Windows image using the supported DISM PowerShell provider.' -Actions @($action) -SourceRecords @('SRC:d9c8da5094')
}
...
function New-WinCareOfflineDriverRemovePlan {
    ...
    $action=New-WinCareAction -Type OfflineImageRemoveDriver -Label "Remove offline driver: $PublishedName" -Risk High -Parameters @{ImagePath=$image;PublishedName=$PublishedName} -RequiresAdmin $true -Reversible $false -Tags @('OfflineImage','Driver','Irreversible') -SourceRecords @('SRC:d9c8da5094') -RecoveryDescription 'No automatic rollback. Export the source driver before removal and remount/reapply if needed.' -TimeoutSeconds 21600
    New-WinCarePlan -Title 'Remove driver from offline image' -Description 'Remove a non-inbox, non-boot-critical OEM driver package.' -Actions @($action) -RollbackOnFailure $false -SourceRecords @('SRC:d9c8da5094')
}
...
function New-WinCareOfflinePackageAddPlan {
    ...
    $action=New-WinCareAction -Type OfflineImageAddPackage -Label "Add package to offline image: $([IO.Path]::GetFileName($package))" -Risk Critical -Parameters @{ImagePath=$image;PackagePath=$package;IgnoreCheck=[bool]$IgnoreCheck} -RequiresAdmin $true -Reversible $false -Tags @('OfflineImage','Package','Irreversible') -SourceRecords @('SRC:d9c8da5094') -RecoveryDescription 'No generic rollback is promised; discard the mounted image or restore its captured image backup.' -RestartPossible $true -TimeoutSeconds 21600
    New-WinCarePlan -Title 'Add package to offline image' -Description 'Apply a CAB or MSU package to a mounted image. This is intentionally non-reversible.' -Actions @($action) -RollbackOnFailure $false -SourceRecords @('SRC:d9c8da5094')
}
...
function New-WinCareOfflineFeaturePlan {
    ...
    if($before -eq $State){return New-WinCarePlan -Title 'Offline feature already in desired state' -Description "$FeatureName is already $State." -SourceRecords @('SRC:d9c8da5094')}
    $action=New-WinCareAction -Type OfflineImageSetFeature -Label "$State offline feature: $FeatureName" -Risk High -Parameters @{ImagePath=$image;FeatureName=$FeatureName;State=$State;BeforeState=$before} -RequiresAdmin $true -Reversible $true -Compensator @{Type='OfflineImageSetFeature';RequiresAdmin=$true;Parameters=@{ImagePath=$image;FeatureName=$FeatureName;State=$before;BeforeState=$State}} -Tags @('OfflineImage','Feature','Idempotent') -SourceRecords @('SRC:d9c8da5094') -RecoveryDescription "Restore feature state to $before." -RestartPossible $true -TimeoutSeconds 21600
    New-WinCarePlan -Title "$State offline Windows feature" -Description 'Change a supported optional feature through the DISM PowerShell provider.' -Actions @($action) -SourceRecords @('SRC:d9c8da5094')
}
