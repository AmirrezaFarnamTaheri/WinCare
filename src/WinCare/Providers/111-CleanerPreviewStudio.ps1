function Get-WinCareCleanerPreviewRoot {
    [CmdletBinding()]param()
    Ensure-WinCareState
    Get-WinCareBridgeStateRoot 'CleanerPreviewStudio'
}
...
function Get-WinCareDiskPressureAssessment {
    ...
    if(-not $IsWindows){return [pscustomobject]@{Available=$false;Reason='WindowsRequired';SourceRecords=@('SRC:65dbd15e56')}}
    ...
    # ponytail: disk pressure threshold profile -> dynamic machine-learned disk space optimizer
...
function New-WinCareDiskPressureCleanupPlan {
    ...
    $plan.SourceRecords=@('SRC:65dbd15e56')
...
function Import-WinCareWinapp2Catalog {
    ...
    if($item.PSIsContainer -or ($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -or $item.Length -gt 2097152){throw 'Winapp2 input must be a regular non-reparse file no larger than 2 MiB.'}
    ...
    foreach($raw in Read-WinCareBoundedLines -LiteralPath $path -MaximumBytes 2097152 -MaximumLines 50000 -MaximumLineCharacters 4096){
        $lineNumber++
    ...
    [pscustomobject]@{SchemaVersion=1;Path=$path;Sha256=Get-WinCareSha256 $path;Accepted=@($accepted);Rejected=@($rejected);AllowedRoots=@($AllowedRoots);Execution='None';SourceRecords=@('SRC:65dbd15e56')}
}
...
function Get-WinCareApplicationDataRelocationProfile {
    ...
    foreach($profile in $profiles){$profile|Add-Member -NotePropertyName SourceRecords -NotePropertyValue @('SRC:65dbd15e56') -Force}
...
function Get-WinCareFilePreview {
    ...
    if($item.PSIsContainer){
        ...
        return [pscustomobject]@{SchemaVersion=1;Path=$path;Kind='Directory';Execution='None';Bytes=$measure.Bytes;Files=$measure.Files;Directories=$measure.Directories;Children=$children;Truncated=($children.Count -ge 200);SourceRecords=@('SRC:65dbd15e56')}
    }
...
function New-WinCareFilePreviewExportPlan {
    ...
    $action=New-WinCareManagedFileWriteAction -Path $path -Content $content -AllowedRoots @($root) -Label 'Write bounded local file preview' -SourceRecords @('SRC:65dbd15e56')
...
function Get-WinCareCleanerPreviewCapabilityCard {
    ...
    $cards=@(
        [pscustomobject]@{Id='disk-pressure';Title='Disk-pressure cleanup';Summary='Threshold-aware cleanup profiles over canonical WinCare targets plus scheduled automation.';Risk='Moderate';SourceRecords=@('SRC:65dbd15e56')},
        [pscustomobject]@{Id='winapp2';Title='Winapp2 admission';Summary='Bounded FileKey-only parser and exact file manifest; registry directives and broad scans are rejected.';Risk='High';SourceRecords=@('SRC:65dbd15e56')},
        [pscustomobject]@{Id='relocation';Title='Verified application-data relocation';Summary='Copy, hash verification, junction commit, internal rollback, and exact recovery without process termination.';Risk='High';SourceRecords=@('SRC:65dbd15e56')},
        [pscustomobject]@{Id='file-preview';Title='Local file preview';Summary='Priority-based, bounded, redacted, non-executing inspection of directories, text, tables, archives, images, PE files, and PDFs.';Risk='ReadOnly';SourceRecords=@('SRC:65dbd15e56')},
        [pscustomobject]@{Id='preview-handlers';Title='Preview-handler assessment';Summary='Inventory registered COM preview handlers and signatures without loading them.';Risk='ReadOnly';SourceRecords=@('SRC:65dbd15e56')}
    )
