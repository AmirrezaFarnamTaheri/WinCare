...
function New-WinCareRemoteSupportExpiryReconciliationPlan {
    ...
    $action=New-WinCareAction -Type 'ReplaceRemoteConsentStore' -Label "Reconcile $changed expired remote-support consent record(s)" -Risk Moderate -Parameters @{Store=$after;ExpectedCurrentHash=$beforeHash} -RequiresAdmin $false -Reversible $true -Compensator @{Type='ReplaceRemoteConsentStore';Parameters=@{Store=$before;ExpectedCurrentHash=$afterHash}} -TimeoutSeconds 30 -Tags @('RemoteSupport','Consent','Reconciliation') -SourceRecords @('W3-REMOTE-CONSENT-LIFECYCLE') -RecoveryDescription 'Restore the exact prior consent-record store.'
    New-WinCarePlan -Title 'Reconcile expired remote-support consent records' -Actions @($action) -SourceRecords @('W3-REMOTE-CONSENT-LIFECYCLE')
}
...
function New-WinCareRemoteSupportConsentStatePlan {
    ...
    $action=New-WinCareAction -Type 'ReplaceRemoteConsentStore' -Label "Set remote-support consent to $State" -Risk Moderate -Parameters @{Store=$after;ExpectedCurrentHash=$beforeHash} -RequiresAdmin $false -Reversible $true -Compensator @{Type='ReplaceRemoteConsentStore';Parameters=@{Store=$before;ExpectedCurrentHash=$afterHash}} -TimeoutSeconds 30 -Tags @('RemoteSupport','Consent') -SourceRecords @('W3-REMOTE-CONSENT-LIFECYCLE') -RecoveryDescription 'Restore the exact prior local consent-record store.'
    New-WinCarePlan -Title 'Change remote-support consent state' -Actions @($action) -SourceRecords @('W3-REMOTE-CONSENT-LIFECYCLE')
}
...
function New-WinCareRemoteSupportEmergencyPlan {
    ...
    $actions=[Collections.Generic.List[object]]::new();$actions.Add((New-WinCareAction -Type 'ReleaseLocalInput' -Label 'Release local cursor and modifier state' -Risk Low -Parameters @{} -RequiresAdmin $false -Reversible $false -TimeoutSeconds 15 -Tags @('RemoteSupport','EmergencyRelease') -SourceRecords @('W3-REMOTE-EMERGENCY-RELEASE','W3-EXPLICIT-CONTROL-HANDOFF')))
    if($TerminateKnownProcesses){
        ...
        foreach($id in @($ProcessId|Sort-Object -Unique)){
            ...
            $actions.Add((New-WinCareAction -Type 'TerminateRemoteSupportProcess' -Label "Terminate remote-support process $($process.Name) ($id)" -Risk High -Parameters @{ProcessId=[int]$id;Name=[string]$process.Name;StartTime=[string]$process.StartTime;Path=[string]$process.Path;Sha256=[string]$process.Sha256} -RequiresAdmin $false -Reversible $false -TimeoutSeconds 30 -Tags @('RemoteSupport','EmergencyRelease') -SourceRecords @('W3-REMOTE-EMERGENCY-RELEASE','W3-RUSTDESK-SESSION-GOVERNANCE') -RecoveryDescription 'A terminated remote-support process is not restarted automatically.'))
        }
    }
    New-WinCarePlan -Title 'Emergency remote-support release' -Description 'Releases local input constraints first, then optionally terminates only exact inventoried remote-support processes.' -Actions @($actions) -RollbackOnFailure $false -SourceRecords @('W3-REMOTE-EMERGENCY-RELEASE','W3-EXPLICIT-CONTROL-HANDOFF')
}
