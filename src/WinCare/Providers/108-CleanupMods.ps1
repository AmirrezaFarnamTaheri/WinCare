function New-WinCareExplorerQuickPlan {
    ...
    $specs=@(
        @{Bound=$PSBoundParameters.ContainsKey('ShowExtensions');Name='HideFileExt';Value=if($ShowExtensions){0}else{1};Label=$(if($ShowExtensions){'Show file extensions'}else{'Hide file extensions'});Source='SRC:6e8ae24ac4'},
        @{Bound=$PSBoundParameters.ContainsKey('ShowHidden');Name='Hidden';Value=if($ShowHidden){1}else{2};Label=$(if($ShowHidden){'Show hidden files'}else{'Hide hidden files'});Source='SRC:6e8ae24ac4'},
        @{Bound=$PSBoundParameters.ContainsKey('LaunchThisPc');Name='LaunchTo';Value=if($LaunchThisPc){1}else{2};Label=$(if($LaunchThisPc){'Open Explorer to This PC'}else{'Open Explorer to Home'});Source='SRC:6e8ae24ac4'}
    )
    foreach($spec in $specs){
        ...
        $actions.Add((New-WinCareAction -Type SetRegistryValue -Label ([string]$spec.Label) -Risk Low -Parameters @{Path=$path;Name=[string]$spec.Name;Value=[int]$spec.Value;ValueType='DWord';Before=$before} -Reversible $true -Compensator $compensator -Tags @('Explorer','Visibility','GovernedConfiguration') -SourceRecords @([string]$spec.Source) -RecoveryDescription 'Restore the exact previous Explorer preference. Explorer may need to be reopened.'))
    }
    New-WinCarePlan -Title 'Explorer quick settings' -Description 'Applies explicit Explorer visibility and launch preferences without installing global hotkeys or terminating Explorer.' -Actions @($actions) -SourceRecords @('SRC:6e8ae24ac4')
}

function Get-WinCareDeepCleanupProfile {
    [CmdletBinding()]param([string]$Id='')
    $profiles=@(
        [pscustomobject]@{Id='bounded-safe-deep-clean';Title='Bounded safe deep clean';Risk='High';Description='Cleans bounded temporary, crash, cache, and diagnostic-log locations while preserving event logs, Prefetch, Defender evidence, and drive-root files.';TargetIds=@('user-temp','windows-temp','thumbnails','icon-cache','d3d-cache','psreadline-history','crash-dumps','windows-minidumps','windows-caches','wer-archive','wer-queue','wer-temp','locallow-temp','cbs-persist-logs','mosetup-logs','panther-logs','webcache-logs','inetcache-logs');RecycleBin=$true;WindowsUpdateCache=$false;ComponentCleanup=$false;SourceRecords=@('SRC:6e8ae24ac4')},
        [pscustomobject]@{Id='windows-cleaner-utility-all';Title='Windows Cleaner Utility — bounded aggressive';Risk='Critical';Description='Executes an aggressive cleanup profile through bounded WinCare targets, coordinated Windows Update cache deletion, Recycle Bin cleanup, and supported component-store cleanup. Root-drive sweeps, event-log deletion, Prefetch purge, Defender evidence deletion, and network reset remain excluded.';TargetIds=@('user-temp','windows-temp','thumbnails','icon-cache','d3d-cache','psreadline-history','crash-dumps','windows-minidumps','memory-dump','windows-caches','wer-archive','wer-queue','wer-temp','locallow-temp','cbs-persist-logs','mosetup-logs','panther-logs','webcache-logs','inetcache-logs');RecycleBin=$true;WindowsUpdateCache=$true;ComponentCleanup=$true;SourceRecords=@('SRC:6e8ae24ac4')}
    )
...
function Import-WinCareAppxSelectionList {
    ...
    $lines=Read-WinCareBoundedLines `
        -LiteralPath $resolved `
        -MaximumBytes 65536 `
        -MaximumLines 1000 `
        -MaximumLineCharacters 512
    $patterns=@(
        $lines |
    ...
    [pscustomobject]@{Path=$resolved;Sha256=Get-WinCareSha256 -LiteralPath $resolved;Patterns=$patterns}
}
...
function Get-WinCareAppxSelectionProfile {
    ...
    $profiles=@(
        [pscustomobject]@{Id='windows-10-curated';Title='Windows 10 curated Store-app removal';Risk='High';Mode='Include';Patterns=$w10;Description='Removes the curated Windows 10 application set.';SourceRecords=@('SRC:6e8ae24ac4')},
        [pscustomobject]@{Id='windows-11-curated';Title='Windows 11 curated Store-app removal';Risk='High';Mode='Include';Patterns=$w11;Description='Removes the curated Windows 11 application set.';SourceRecords=@('SRC:6e8ae24ac4')},
        [pscustomobject]@{Id='keep-core-apps-only';Title='Keep core apps only';Risk='Critical';Mode='Exclude';Patterns=$core;Description='Allowlist mode: remove every removable application that is not in the WinCare core keep set.';SourceRecords=@('SRC:6e8ae24ac4')},
        [pscustomobject]@{Id='remove-every-removable-appx';Title='Remove every removable Store application';Risk='Critical';Mode='Include';Patterns=@('*');Description='Removes every package WinCare can identify as removable, including Store and recovery-facing apps. Framework, resource, and explicitly non-removable packages are excluded.';SourceRecords=@('SRC:6e8ae24ac4');AllowMatchAll=$true},
        [pscustomobject]@{Id='remove-copilot';Title='Remove Copilot packages';Risk='High';Mode='Include';Patterns=@('*Microsoft.Copilot*','MicrosoftWindows.Client.WebExperience');Description='Removes Copilot and the Web Experience package when removable.';SourceRecords=@('SRC:6e8ae24ac4')}
    )
...
function New-WinCareOfflineAppxSelectionPlan {
    ...
    if($ProfileId){$profile=Get-WinCareAppxSelectionProfile -Id $ProfileId|Select-Object -First 1;if(-not $profile){throw "Unknown AppX selection profile: $ProfileId"};$Pattern=@($profile.Patterns);$Mode=[string]$profile.Mode;$risk=[string]$profile.Risk;$sources=@($profile.SourceRecords);$title=[string]$profile.Title}else{$profile=$null;$risk='Critical';$sources=@('SRC:6e8ae24ac4');$title='Custom offline AppX selection'}
    ...
    New-WinCarePlan -Title "Offline $title" -Description "Removes $($actions.Count) provisioned package(s) from the already mounted read-write image. WinCare does not mount or commit a WIM implicitly." -Actions $actions -RollbackOnFailure $false -Metadata $metadata -SourceRecords @($sources+'SRC:6e8ae24ac4')
}
