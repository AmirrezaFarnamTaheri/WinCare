function Get-WinCareWorkspaceLayoutRoot { Ensure-WinCareState;$root=Join-Path $script:WinCareState.Root 'WorkspaceLayouts';$null=New-Item -ItemType Directory -Path $root -Force;$root }
...
function New-WinCareWorkspaceLayoutRecord {
    ...
    $record=[ordered]@{SchemaVersion=1;Id=$Id;Name=$Name;Description=$Description;CreatedAt=$now;UpdatedAt=$now;Slots=$slots;SourceRecords=@('D62-SNAPZONES-LAYOUTS','D58-TABAME-WINDOW-ZONES','D60-PINPOINT-LAUNCH-LAYOUT')};$null=Test-WinCareWorkspaceLayoutRecord $record;[pscustomobject]$record
}
...
function New-WinCareWorkspaceLayoutSavePlan {
    ...
    $action=New-WinCareAction -Type ReplaceWorkspaceLayoutStore -Label "Save workspace layout: $($Layout.Name)" -Risk Low -Parameters @{Store=$after;ExpectedCurrentHash=Get-WinCareWorkspaceLayoutStoreHash $store} -Reversible $true -Compensator @{Type='ReplaceWorkspaceLayoutStore';RequiresAdmin=$false;Parameters=@{Store=$before;ExpectedCurrentHash=Get-WinCareWorkspaceLayoutStoreHash $after}} -Tags @('Windows','Layouts','CAS') -SourceRecords @('D62-SNAPZONES-LAYOUTS','D58-TABAME-WINDOW-ZONES') -RecoveryDescription 'Restore the exact previous layout catalog.'
    New-WinCarePlan -Title 'Save workspace layout' -Actions @($action) -SourceRecords @('D62-SNAPZONES-LAYOUTS','D58-TABAME-WINDOW-ZONES')
}
...
function New-WinCareWorkspaceLayoutRemovePlan {
    ...
    $action=New-WinCareAction -Type ReplaceWorkspaceLayoutStore -Label "Remove workspace layout: $($record.Name)" -Risk Moderate -Parameters @{Store=$after;ExpectedCurrentHash=Get-WinCareWorkspaceLayoutStoreHash $store} -Reversible $true -Compensator @{Type='ReplaceWorkspaceLayoutStore';RequiresAdmin=$false;Parameters=@{Store=$before;ExpectedCurrentHash=Get-WinCareWorkspaceLayoutStoreHash $after}} -Tags @('Windows','Layouts','CAS') -SourceRecords @('D62-SNAPZONES-LAYOUTS') -RecoveryDescription 'Restore the exact removed layout.'
    New-WinCarePlan -Title 'Remove workspace layout' -Actions @($action) -SourceRecords @('D62-SNAPZONES-LAYOUTS')
}
