...
function Resolve-WinCarePhysicalDiskForDrive {
    ...
    try {
        ...
        if($null -eq $partition) { throw "No partition is associated with drive $DriveLetter`:" }
        ...
        if(-not [string]::IsNullOrWhiteSpace($logicalUniqueId)) {
            $uniqueMatches=@($physicalDisks|Where-Object { [string]$_.UniqueId -eq $logicalUniqueId })
            if($uniqueMatches.Count -eq 1) { $candidate=$uniqueMatches[0] }
        }
        if($null -eq $candidate) {
            $deviceMatches=@($physicalDisks|Where-Object { [string]$_.DeviceId -eq [string]$diskNumber })
            if($deviceMatches.Count -eq 1) { $candidate=$deviceMatches[0] }
        }
        if($null -eq $candidate -and -not [string]::IsNullOrWhiteSpace([string]$logicalDisk.FriendlyName)) {
            $nameMatches=@($physicalDisks|Where-Object { [string]$_.FriendlyName -eq [string]$logicalDisk.FriendlyName })
            if($nameMatches.Count -eq 1) { $candidate=$nameMatches[0] }
        }
        if($null -eq $candidate) {
            return [pscustomobject]@{
                Success=$false
                Status='DiskIdentityUnresolved'
                Reason="The physical disk backing drive $DriveLetter`: could not be resolved unambiguously."
...
function Get-WinCareStorageReliabilityEvidence {
    ...
    if(-not (Get-Command Get-StorageReliabilityCounter -ErrorAction SilentlyContinue)) {
        return [pscustomobject]@{
            Success=$false
            Status='ProviderUnavailable'
    ...
    try {
        ...
        if($null -eq $counter) { throw 'The storage provider returned no reliability counter record.' }
        [pscustomobject]@{
            Success=$true
            Status='Measured'
            Reason=$null
...
function Get-WinCareStorageHealthTriage {
    ...
    if($cleanDrive -notmatch '^[A-Za-z]$') {
        throw 'DriveLetter must identify one drive letter, for example C, C:, or C:\.'
    }
    $cleanDrive=$cleanDrive.ToUpperInvariant()
    $issues=[Collections.Generic.List[string]]::new()
    $resolution=Resolve-WinCarePhysicalDiskForDrive -DriveLetter $cleanDrive

    if(-not $resolution.Success) {
        if(-not [string]::IsNullOrWhiteSpace([string]$resolution.Reason)) { $issues.Add([string]$resolution.Reason) }
        return [pscustomobject]@{
            Supported=$false
    ...
    $counterEvidence=Get-WinCareStorageReliabilityEvidence -PhysicalDisk $physicalDisk
    if(-not $counterEvidence.Success -and -not [string]::IsNullOrWhiteSpace([string]$counterEvidence.Reason)) {
        $issues.Add([string]$counterEvidence.Reason)
    }
    $counter=$counterEvidence.Counter

    $wear=if($counter -and $counter.PSObject.Properties['Wear'] -and $null -ne $counter.Wear){[int]$counter.Wear}else{$null}
    $temp=if($counter -and $counter.PSObject.Properties['Temperature'] -and $null -ne $counter.Temperature){[int]$counter.Temperature}else{$null}
    $uncorrected=if($counter -and $counter.PSObject.Properties['ReadErrorsTotal'] -and $null -ne $counter.ReadErrorsTotal){[long]$counter.ReadErrorsTotal}else{$null}
    if($null -eq $wear){$issues.Add('WearPercentageUnavailable')}
    if($null -eq $temp){$issues.Add('TemperatureUnavailable')}
    if($null -eq $uncorrected){$issues.Add('ReadErrorsTotalUnavailable')}
    ...
    $elevated=(($null -ne $wear -and $wear -gt 80) -or
        ($null -ne $temp -and $temp -gt 70) -or
        ($null -ne $uncorrected -and $uncorrected -gt 0))
    $complete=($null -ne $wear -and $null -ne $temp -and $null -ne $uncorrected)
    $healthStatus=if($elevated){'ElevatedRisk'}elseif($complete){'Healthy'}else{'Unknown'}
    ...
    $completeness=if($complete -and $null -ne $mediaType){'Complete'}else{'Partial'}
    ...
    [pscustomobject]@{
        ...
        PhysicalDiskUniqueId=if($physicalDisk.PSObject.Properties['UniqueId']){[string]$physicalDisk.UniqueId}else{$null}
        PhysicalDiskFriendlyName=if($physicalDisk.PSObject.Properties['FriendlyName']){[string]$physicalDisk.FriendlyName}else{$null}
        MediaType=$mediaType
