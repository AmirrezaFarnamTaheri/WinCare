function Get-WinCareAutomationRoot {
    $root=Join-Path $script:WinCareState.Root 'Automation'
...
function New-WinCareAutomationTaskPlan {
    ...
    $action=New-WinCareAction -Type $type -Label "$(if($Remove){'Remove'}else{'Register'}) automation $($Profile.title)" -Risk Moderate -Parameters @{Profile=$Profile} -RequiresAdmin $false -Reversible (-not $Remove) -Compensator $(if(-not $Remove){@{Type='RemoveAutomationTask';Parameters=@{Profile=$Profile}}}else{@{}}) -Tags @('Automation','Idempotent') -SourceRecords @('D09-DECLARATIVE-RESOURCES','D08-MAINTENANCE-ORCHESTRATION')
    New-WinCarePlan -Title 'Configure scheduled maintenance' -Actions @($action) -SourceRecords @('D09-DECLARATIVE-RESOURCES','D08-MAINTENANCE-ORCHESTRATION')
}
