...
function New-WinCareMaintenanceWindowRecord {
    ...
    param(
        ...
        [string[]]$SourceRecords=@('D24-MAINTENANCE-LIFECYCLE','D25-METRICS')
    )
...
function New-WinCareMaintenanceMetricsExportPlan {
    ...
    $action=New-WinCareManagedFileWriteAction -Path $path -Content $content -AllowedRoots @($root) -Label 'Export maintenance metrics' -SourceRecords @('D25-PROMETHEUS-EXPORTER')
    New-WinCarePlan -Title 'Export maintenance metrics' -Description 'Write a local Prometheus text exposition file; no network listener or telemetry is created.' -Actions @($action) -SourceRecords @('D25-PROMETHEUS-EXPORTER')
}
