...
function New-WinCareMasterVolumePlan {
    ...
    $action=New-WinCareAction -Type SetMasterVolume -Label "Set master volume to $Percent%" -Risk Low -Parameters $parameters -Reversible $true -Compensator @{Type='SetMasterVolume';Parameters=@{Percent=$beforePercent;Mute=$beforeMute}} -Tags @('DesktopControl','Idempotent') -SourceRecords @('D10-QUICK-CONTROLS')
    New-WinCarePlan -Title 'Change master audio' -Actions @($action) -SourceRecords @('D10-QUICK-CONTROLS')
}
...
function New-WinCareClearClipboardPlan {
    $action=New-WinCareAction -Type ClearClipboard -Label 'Clear the current clipboard contents' -Risk Moderate -Parameters @{} -Reversible $false -Tags @('Privacy','Clipboard') -SourceRecords @('D10-CLIPBOARD-PRIVACY') -RecoveryDescription 'Clipboard contents cannot be restored by WinCare.'
    New-WinCarePlan -Title 'Clear clipboard' -Actions @($action) -RollbackOnFailure $false -SourceRecords @('D10-CLIPBOARD-PRIVACY')
}
...
function Open-WinCareDisplaySettings { Start-Process 'ms-settings:display' }
function Open-WinCareSoundSettings { Start-Process 'ms-settings:sound' }
