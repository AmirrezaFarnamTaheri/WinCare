...
function New-WinCareOpenLaunchTargetPlan {
    ...
    $action=New-WinCareAction -Type OpenLaunchTarget -Label "Open: $($target.Name)" -Risk Low -Parameters @{Id=$target.Id;Kind=$target.Kind;Target=$target.Target;ExpectedHash=$target.TargetHash} -Tags @('Launcher','Explicit') -SourceRecords @('D58-LAUNCHER','D60-PLUGIN-SEARCH','D61-SECURE-LAUNCHER','D63-COMMAND-PALETTE') -TimeoutSeconds 60
    New-WinCarePlan -Title "Open $($target.Name)" -Description 'Open an exact target from the current local launcher index. Arbitrary command strings are not accepted.' -Actions @($action) -SourceRecords @('D58-LAUNCHER','D60-PLUGIN-SEARCH','D61-SECURE-LAUNCHER','D63-COMMAND-PALETTE')
}
...
function Invoke-WinCareOpenLaunchTargetAction {
    ...
    try{
        switch([string]$target.Kind){
            'Shortcut'{$result=Start-Process -FilePath $target.Target -PassThru -ErrorAction Stop}
            'Executable'{$result=Start-Process -FilePath $target.Target -PassThru -ErrorAction Stop}
