function Get-WinCareLocalNotesRoot {
    Ensure-WinCareState
    $root=Join-Path $script:WinCareState.Root 'Notes';$null=New-Item -ItemType Directory -Path $root -Force;$root
}
...
function Get-WinCareLocalNoteSnapshot {
    ...
    if($hasMetadata){
        ...
        $body=Get-Content -LiteralPath $paths.Body -Raw -Encoding utf8 -ErrorAction Stop
    }
...
function New-WinCareLocalNotePlan {
    ...
    param(
        ...
        [string[]]$SourceRecords=@('W3-LOCALNOTES-PLAIN-MARKDOWN')
    )
...
function New-WinCareLocalNoteRemovePlan {
    ...
    $action=New-WinCareAction -Type 'RemoveLocalNote' -Label "Remove local note: $($before.Metadata.Title)" -Risk Moderate -Parameters @{Id=$Id;ExpectedBeforeHash=$before.Hash} -RequiresAdmin $false -Reversible $true -Compensator @{Type='RestoreLocalNote';Parameters=@{Id=$Id;Snapshot=$before;ExpectedCurrentHash=$after.Hash}} -TimeoutSeconds 30 -Tags @('Notes','LocalFirst','Markdown') -SourceRecords @('W3-LOCALNOTES-PLAIN-MARKDOWN') -RecoveryDescription 'Restore the exact removed Markdown note and metadata.'
    New-WinCarePlan -Title 'Remove local note' -Actions @($action) -SourceRecords @('W3-LOCALNOTES-PLAIN-MARKDOWN')
}
