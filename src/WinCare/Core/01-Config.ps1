...
function Initialize-WinCareState {
    ...
    foreach ($name in @('Logs','Journal','Backups','Reports','Quarantine','Cache','Extensions','Automation','Broker','Provisioning','Maintenance','Widgets','Customization','Metrics','Playbooks','Images','SecurityMaintenance','NetworkExperiments','Instrumentation','PowerSessions','ColorStudio','Notes','RemoteSupport','Browser','Downloads','TelemetrySnapshots','Launcher','GameState','WorkspaceLayouts','ContainerLogs','WorkspaceStudio')) {
    ...
    if (Test-Path -LiteralPath $configPath) {
        try {
            ...
            $corrupt="$configPath.corrupt.$([datetime]::UtcNow.ToString('yyyyMMddHHmmss'))"
            Move-Item -LiteralPath $configPath -Destination $corrupt -Force
            $config=Get-WinCareDefaultConfig
        }
    }
    $persisted=[ordered]@{}; foreach ($key in $config.Keys) {$persisted[$key]=$config[$key]}
    $script:WinCareState=@{
        Root=$root; ConfigPath=$configPath; Config=$config; PersistedConfig=$persisted
        SessionOverrides=[Collections.Generic.HashSet[string]]::new([StringComparer]::OrdinalIgnoreCase)
        ReadOnlyLocked=[bool]($PSBoundParameters.ContainsKey('ReadOnly') -and $ReadOnly)
        IsAdmin=Test-WinCareAdministrator; IsWindows=$IsWindows; StartedAt=[datetime]::UtcNow
        SessionId=[guid]::NewGuid().ToString('N'); Capabilities=@{}; Navigation=@{}; StatusBadges=@{}
    }
    if ($PSBoundParameters.ContainsKey('Ascii')) {$script:WinCareState.Config.Ascii=[bool]$Ascii;$null=$script:WinCareState.SessionOverrides.Add('Ascii')}
    if ($PSBoundParameters.ContainsKey('ReadOnly')) {$script:WinCareState.Config.ReadOnly=[bool]$ReadOnly;$null=$script:WinCareState.SessionOverrides.Add('ReadOnly')}
    if ($PSBoundParameters.ContainsKey('Theme')) {$script:WinCareState.Config.Theme=$Theme;$null=$script:WinCareState.SessionOverrides.Add('Theme')}
    $script:WinCareState.Policy=Get-WinCareEffectivePolicy
    if ($script:WinCareState.Policy.ReadOnly) {$script:WinCareState.Config.ReadOnly=$true;$script:WinCareState.ReadOnlyLocked=$true}
    Initialize-WinCareCapabilities
    if(Get-Command Repair-WinCareInterruptedOperation -ErrorAction SilentlyContinue){$script:WinCareState.InterruptedOperations=@(Repair-WinCareInterruptedOperation)}
    if (-not $SkipConfigSave) { Save-WinCareConfig }
}
