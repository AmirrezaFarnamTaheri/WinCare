...
function Repair-WinCareInterruptedOperation {
    ...
    foreach($directory in Get-ChildItem -LiteralPath $root -Directory -ErrorAction SilentlyContinue){
        ...
        try{
            $record=Get-Content -LiteralPath $path -Raw|ConvertFrom-Json -AsHashtable -Depth 50
...
function Request-WinCareOperationCancellation {
    ...
    try{$record=Get-Content -LiteralPath $recordPath -Raw|ConvertFrom-Json -AsHashtable -Depth 50}catch{return New-WinCareResult -Success $false -Status Failed -Code 'OperationJournalInvalid' -Message $_.Exception.Message -ExitCode 74}
...
function Invoke-WinCareAction {
    ...
    if(-not $Rollback -and (Test-WinCareActionAlreadyApplied -Action $Action)){return New-WinCareResult -Success $true -Status Succeeded -Code 'AlreadyAppliedAndVerified' -Message 'The idempotent action is already applied and its postconditions were verified.' -OperationId $OperationId -ActionId $Action.Id}
    if($Action.RequiresAdmin -and -not $script:WinCareState.IsAdmin){return Invoke-WinCareElevatedAction -Action $Action -OperationId $OperationId -TimeoutSeconds ([int]$Action.TimeoutSeconds)}
