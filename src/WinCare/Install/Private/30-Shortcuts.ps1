# wincare.brand.shortcut-icon/v1
...
function Test-WinCareShortcutIdentity {
    ...
    try{$shell=New-Object -ComObject WScript.Shell;$shortcut=$shell.CreateShortcut($ShortcutPath);([IO.Path]::GetFullPath($shortcut.TargetPath) -eq [IO.Path]::GetFullPath($ExpectedTarget)) -and ([string]$shortcut.Arguments -eq $ExpectedArguments) -and ([IO.Path]::GetFullPath($shortcut.WorkingDirectory) -eq [IO.Path]::GetFullPath($ExpectedWorkingDirectory)) -and ([string]::IsNullOrWhiteSpace($ExpectedIconLocation) -or [string]::Equals([string]$shortcut.IconLocation,$ExpectedIconLocation,[StringComparison]::OrdinalIgnoreCase))}catch{$false}finally{if($shortcut){[void][Runtime.InteropServices.Marshal]::FinalReleaseComObject($shortcut)};if($shell){[void][Runtime.InteropServices.Marshal]::FinalReleaseComObject($shell)}}
}
...
function Install-WinCareShortcuts {
    ...
    try{$shell=New-Object -ComObject WScript.Shell;foreach($r in $records){$shortcut=$null;try{$shortcut=$shell.CreateShortcut((Join-Path $folder $r.Name));$shortcut.TargetPath=$r.Target;$shortcut.Arguments=$r.Arguments;$shortcut.WorkingDirectory=$r.WorkingDirectory;$shortcut.IconLocation=$r.IconLocation;$shortcut.Save()}finally{if($shortcut){[void][Runtime.InteropServices.Marshal]::FinalReleaseComObject($shortcut)}}}}finally{if($shell){[void][Runtime.InteropServices.Marshal]::FinalReleaseComObject($shell)}}
    foreach($r in $records){if(!(Test-WinCareShortcutIdentity (Join-Path $folder $r.Name) $r.Target $r.Arguments $r.WorkingDirectory $r.IconLocation)){throw "Created shortcut failed identity verification: $($r.Name)"}}
...
function Move-WinCareOwnedShortcutsToBackup {
    ...
    foreach($r in $owner.Shortcuts){$arguments=if($null -eq $r.Arguments){''}else{[string]$r.Arguments};$working=if($r.WorkingDirectory){[string]$r.WorkingDirectory}else{$Destination};$iconLocation=if($r.PSObject.Properties['IconLocation']){[string]$r.IconLocation}else{$null};if(!(Test-WinCareShortcutIdentity (Join-Path $folder $r.Name) $r.Target $arguments $working $iconLocation)){throw "Managed shortcut identity mismatch: $($r.Name)"}}
