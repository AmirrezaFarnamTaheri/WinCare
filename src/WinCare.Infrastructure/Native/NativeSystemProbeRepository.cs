...
public sealed class NativeSystemProbeRepository : INativeSystemProbeRepository
{
    ...
    public ValueTask<SystemSnapshot> GetSystemSnapshotAsync(CancellationToken ct = default)
    {
        ...
        unsafe
        {
            ...
            try
            {
                var pwr = Commands.WindowsCommandExecutor.HardwareTunerHelper.ReadPowerStatus();
                if (pwr.Success)
                {
                    batteryPct = pwr.BatteryLifePercent;
                    isCharging = pwr.IsCharging;
                    hasBattery = pwr.HasBattery;
                }
                var plan = Commands.WindowsCommandExecutor.HardwareTunerHelper.GetActivePowerScheme();
                powerPlan = plan.FriendlyName;

                var disp = Commands.WindowsCommandExecutor.HardwareTunerHelper.GetDisplayMetrics();
                if (disp.Success)
                {
                    displayRes = disp.Summary;
                }
            }
            catch
            {
