...
public sealed class StorageReportService : IStorageReportService
{
    ...
    private static StorageReport Scan(StorageReportRequest request, CancellationToken cancellationToken)
    {
        ...
        var directories = new List<DirectoryAccumulator> { new(root, null, -1, 0) };
            ...
            try
            {
                foreach (string path in Directory.EnumerateFileSystemEntries(current.Path))
                {
                    ...
                    if ((attributes & FileAttributes.Directory) != 0)
                    {
                        ...
                        directories.Add(new DirectoryAccumulator(path, current.Path, current.DirectoryIndex, depth));
    ...
    {
        for (int current = index; current >= 0; current = directories[current].ParentIndex)
        {
    ...
    private sealed class DirectoryAccumulator(string path, string? parentPath, int parentIndex, int depth)
    {
