...
internal sealed partial class WindowsCommandExecutor
{
    ...
    private Task<CommandHandlerOutcome> InstallerCacheAnalysisAsync(CommandParameters p, CancellationToken cancellationToken)
    {
        ...
        bool activeInstall = IsMsiExecuteMutexHeld() || HasInstallerInProgressKey();
        bool pendingRenames = HasPendingInstallerRenames();
            ...
            : "Installer cache analysis completed; every cache file remains retained.", new
        {
            ...
            activeInstallationInProgress = activeInstall,
            pendingRebootRenamesDetected = pendingRenames,
    ...
    private static bool IsMsiExecuteMutexHeld()
    {
        try
        {
            return Mutex.TryOpenExisting(@"Global\_MSIExecute", out Mutex? mutex) && mutex is not null;
        }
        catch
        {
            return false;
    ...
    private static bool HasInstallerInProgressKey()
    {
        try
        {
            using RegistryKey? key = Registry.LocalMachine.OpenSubKey(@"SOFTWARE\Microsoft\Windows\CurrentVersion\Installer\InProgress", writable: false);
    ...
    private static bool HasPendingInstallerRenames()
    {
        try
        {
            using RegistryKey? sessionManager = Registry.LocalMachine.OpenSubKey(@"SYSTEM\CurrentControlSet\Control\Session Manager", writable: false);
            if (sessionManager?.GetValue("PendingFileRenameOperations") is string[] renames)
            {
                return renames.Any(r => r.Contains(@"\Installer\", StringComparison.OrdinalIgnoreCase) || r.Contains(@"/Installer/", StringComparison.OrdinalIgnoreCase));
            }
            return false;
        }
        catch
