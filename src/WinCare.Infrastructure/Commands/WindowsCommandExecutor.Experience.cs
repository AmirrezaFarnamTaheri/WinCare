...
internal sealed partial class WindowsCommandExecutor
{
    private CommandHandlerOutcome StaticProfileCatalog(string profileFamily)
    {
        object[] profiles = profileFamily switch
        {
            ...
            [
                ...
                new { id = "event-logs", title = "Windows Event Logs", targets = new[] { "Diagnostics", "Bits", "PowerShell", "CrashDumps" } },
            ],
            "appx-selection" =>
            [
    ...
    private CommandHandlerOutcome HardwareInventory(CancellationToken cancellationToken)
    {
        ...
        return Success("hardware-inventory", "Hardware inventory collected from native Windows/BCL surfaces.", new
        {
            ...
            driverServices = DriverServiceAudit().Data,
        });
    }
    ...
    internal static object[] DeepCleanAffectedResources(CommandParameters p)
    {
        ...
        string resourceType = profile.Equals("event-logs", StringComparison.OrdinalIgnoreCase) ? "EventLog" : "FileSystem";
        string path = profile.Equals("event-logs", StringComparison.OrdinalIgnoreCase)
            ? "wevtutil.exe"
    ...
    internal static class Winapp2CategoryResolver
    {
        private static readonly IReadOnlyDictionary<int, string> Categories = new Dictionary<int, string>
        {
            [3006] = "Microsoft Edge",
            [3021] = "Applications",
            [3022] = "Internet",
        ...
        public static string ResolveCategory(int? langSecRef, string? sectionName)
        {
            if (langSecRef.HasValue && Categories.TryGetValue(langSecRef.Value, out string? cat))
                return cat;
        ...
        public static bool CheckSpecialDetect(string code)
        {
            return code.ToUpperInvariant() switch
            {
                "DET_CHROME" => Directory.Exists(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Google", "Chrome", "User Data")),
                "DET_FIREFOX" => Directory.Exists(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "Mozilla", "Firefox")),
                "DET_EDGE" => Directory.Exists(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Microsoft", "Edge", "User Data")),
                "DET_OPERA" => Directory.Exists(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "Opera Software", "Opera Stable")),
                "DET_THUNDERBIRD" => Directory.Exists(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "Thunderbird")),
                "DET_WINSTORE" => Directory.Exists(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Packages")),
                _ => false,
            };
        }
    }
    ...
    private CommandHandlerOutcome Winapp2Admission(CommandParameters p)
    {
        ...
        int sections = 0;
        bool containsExecutableDirective = false;
        var categories = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        var specialDetects = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        ...
        foreach (string line in File.ReadLines(path))
        {
            ...
            else if (trimmed.StartsWith("Run=", StringComparison.OrdinalIgnoreCase) || trimmed.StartsWith("Command=", StringComparison.OrdinalIgnoreCase))
            {
                containsExecutableDirective = true;
            }
            else if (trimmed.StartsWith("LangSecRef=", StringComparison.OrdinalIgnoreCase) && int.TryParse(trimmed["LangSecRef=".Length..].Trim(), out int langSecRef))
            {
                categories.Add(Winapp2CategoryResolver.ResolveCategory(langSecRef, currentSection));
            }
            else if (trimmed.StartsWith("SpecialDetect=", StringComparison.OrdinalIgnoreCase))
            {
                string code = trimmed["SpecialDetect=".Length..].Trim();
                specialDetects.Add(code);
            }
        }

        bool admitted = sections > 0 && !containsExecutableDirective;
        return Success("cleaner-winapp2", "Winapp2-style INI inspected for admission.", new
        {
            path,
            sections,
            containsExecutableDirective,
            admitted,
            categories = categories.OrderBy(c => c).ToArray(),
            specialDetectCodes = specialDetects.OrderBy(s => s).ToArray(),
        });
    }
    ...
    private CommandHandlerOutcome CleanerWinapp2Run(CommandParameters p, CancellationToken cancellationToken)
    {
        ...
        foreach (string line in File.ReadLines(path))
        {
            ...
            else if (trimmed.StartsWith("ExcludeKey", StringComparison.OrdinalIgnoreCase) && trimmed.Contains('='))
            {
                string value = trimmed[(trimmed.IndexOf('=') + 1)..];
                string[] parts = value.Split('|');
                if (parts.Length >= 2) excludePatterns.Add(parts[1].Trim());
            }
        }
        ...
        foreach (string root in allowedRoots)
        {
            ...
            foreach (string pattern in patterns.Where(IsSafeFilePattern))
            {
                ...
                {
                    cancellationToken.ThrowIfCancellationRequested();
                    if (excludePatterns.Any(ex => file.EndsWith(ex, StringComparison.OrdinalIgnoreCase) || Path.GetFileName(file).Equals(ex, StringComparison.OrdinalIgnoreCase))) continue;
        ...
        return Success("cleaner-winapp2-run", "Admitted Winapp2 file patterns applied only inside WinCare temporary-file boundaries.", new { removed, freedBytes = freed, patterns = patterns.ToArray(), excludedCount = excludePatterns.Count });
    }
    ...
    {
        string profile = p.String("ProfileId", "temp");
        if (profile.Equals("event-logs", StringComparison.OrdinalIgnoreCase))
        {
            return CleanEventLogsAsync(p, cancellationToken).GetAwaiter().GetResult();
        }
        if (!profile.Equals("temp", StringComparison.OrdinalIgnoreCase) && !profile.Equals("wincare-cache", StringComparison.OrdinalIgnoreCase)) throw new CommandParameterException("ProfileId", "ProfileId must be temp, wincare-cache, or event-logs.");
    ...
    internal async Task<CommandHandlerOutcome> CleanEventLogsAsync(CommandParameters p, CancellationToken cancellationToken)
    {
        string channel = p.String("Channel", "Application").Trim();
        string[] targets = channel.Equals("all", StringComparison.OrdinalIgnoreCase)
        ...
        {
            ProcessExecutionResult result = await _process.RunAsync("wevtutil.exe", ["cl", target], cancellationToken).ConfigureAwait(false);
            results.Add(new { channel = target, exitCode = result.ExitCode, success = result.ExitCode == 0 });
        }

        return Success("clean-event-logs", $"Windows event log cleanup executed for {results.Count} channel(s).", new
        {
            channel,
            channels = results
        }, undo: false);
