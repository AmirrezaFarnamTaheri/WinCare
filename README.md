...
|  - High-contrast Cyber-Teal theme  |  - Dynamic Plugin Host           |  - Named-pipe Guard Daemon |
...
- **Background Health Daemon (`wincare-guard`)**: Standalone background service performing sub-millisecond RAM pressure, disk quota, and thermal checks, communicating with the UI via secure Named Pipes (`\\.\pipe\WinCareGuardPipe`).
...
Official releases are published automatically via GitHub Actions upon tagged commits and master builds:
