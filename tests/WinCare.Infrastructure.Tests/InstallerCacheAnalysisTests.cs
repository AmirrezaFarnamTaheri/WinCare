...
public sealed class InstallerCacheAnalysisTests
{
    ...
    public async Task Analysis_retains_unmatched_files_and_reports_registration_evidence()
    {
        ...
        Assert.True(outcome.Data!.Value.TryGetProperty("activeInstallationInProgress", out _));
        Assert.True(outcome.Data!.Value.TryGetProperty("pendingRebootRenamesDetected", out _));
    }
