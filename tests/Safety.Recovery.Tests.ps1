...
Describe 'Filesystem confinement and integrity' {
  InModuleScope WinCare {
    ...
    It 'recognizes contained and escaping paths' {
      Test-WinCarePathWithin -Child (Join-Path $TestDrive 'a\b') -Parent $TestDrive | Should -BeTrue
      Test-WinCarePathWithin -Child (Join-Path (Split-Path $TestDrive -Parent) 'escape') -Parent $TestDrive | Should -BeFalse
    }
    ...
    It 'moves a path without overwriting and verifies hash' {
      ...
      (Invoke-WinCareMoveFileAction $action).Success | Should -BeTrue
    ...
    It 'enforces declared roots for move actions' {
      ...
      {Invoke-WinCareMoveFileAction $action} | Should -Throw '*outside approved roots*'
    ...
    It 'rejects a missing destination below a reparse-point ancestor on Windows' -Skip:(-not $IsWindows) {
      ...
      {Assert-WinCareSafePath -LiteralPath (Join-Path $link 'missing\child.txt') -AllowedRoots @($TestDrive) -AllowMissing}|Should -Throw '*Reparse points are not allowed*'
    }
    It 'creates a cancellation request only for an active journal' {
      ...
      Test-Path (Join-Path $root cancel)|Should -BeTrue
    ...
    It 'requires an authenticated receipt for every terminal journal' {
      ...
      (Test-WinCareOperationJournalIntegrity $journal.Root).Valid|Should -BeFalse
      $null=Complete-WinCareOperationReceipt $journal
      (Test-WinCareOperationJournalIntegrity $journal.Root).Valid|Should -BeTrue
    }
    It 'reconciles interrupted active journals through the event chain and terminal receipt' {
      ...
      Test-Path (Join-Path $journal.Root 'receipt.json')|Should -BeTrue
    }
...
Describe 'Condition composition' {
  InModuleScope WinCare {
    ...
    It 'supports all, any, only-one, and not' {
      ...
      Test-WinCareCondition (New-WinCareCondition -Kind All -Children @($t,$t)) @{x=1} | Should -BeTrue
      Test-WinCareCondition (New-WinCareCondition -Kind Any -Children @($f,$t)) @{x=1} | Should -BeTrue
      Test-WinCareCondition (New-WinCareCondition -Kind OnlyOne -Children @($f,$t)) @{x=1} | Should -BeTrue
      Test-WinCareCondition (New-WinCareCondition -Kind Not -Children @($f)) @{x=1} | Should -BeTrue
    }
  }
}
