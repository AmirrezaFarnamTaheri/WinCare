...
Describe 'Wave seven cleanup and AppX convergence' {
    ...
    It 'registers the new typed mutation contracts' {
        ...
        (Get-WinCareActionContract -Type 'RemoveAppxPackageAllUsers').AlwaysAdmin | Should -BeTrue
    ...
    It 'uses bounded wildcard lists rather than donor regex execution' {
    ...
    It 'reserves the bare match-all wildcard for trusted built-in profiles' {
        Test-WinCareAppxSelectionPattern '*' | Should -BeFalse
        Test-WinCareAppxSelectionPattern '*' -AllowMatchAll | Should -BeTrue
        ...
        [bool]$profile.AllowMatchAll | Should -BeTrue
    }
