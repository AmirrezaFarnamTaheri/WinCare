...
Describe 'Wave-three productivity convergence contracts' {
  InModuleScope WinCare {
    ...
    It 'keeps sensitive productivity policy denied by default' {
      $policy=Get-WinCareDefaultPolicy
      $policy.AllowAwayModePowerSession|Should -BeFalse
      $policy.AllowIndefinitePowerSession|Should -BeFalse
      $policy.AllowRemoteSupportTermination|Should -BeFalse
    ...
    It 'preserves local-note privacy while allowing explicit local search' {
      ...
      (Invoke-WinCareRemoveLocalNoteAction -Action $remove.Actions[0]).Success|Should -BeTrue
    }

    It 'rejects tampered local-note recovery snapshots' {
      $snapshot=[ordered]@{Exists=$false;Metadata=$null;Body='';Hash=('0'*64)}
      {Set-WinCareLocalNoteSnapshotInternal -Id '0123456789abcdef0123456789abcdef' -Snapshot $snapshot}|Should -Throw '*hash*'
    }
    ...
    It 'accepts governed palette additions and exact rollback descriptors' {
      ...
      (Invoke-WinCareReplaceColorPaletteStoreAction -Action $plan.Actions[0]).Success|Should -BeTrue
    ...
    It 'models remote-support consent as a closed expiring lifecycle' {
      ...
      (Invoke-WinCareReplaceRemoteConsentStoreAction -Action $plan.Actions[0]).Success|Should -BeTrue
      ...
      (Invoke-WinCareReplaceRemoteConsentStoreAction -Action $active.Actions[0]).Success|Should -BeTrue
      ...
      {New-WinCareRemoteSupportConsentStatePlan -Id $consent.Id -State Closed}|Should -Throw '*Expired consent*'
    ...
    It 'admits only data-only extension command aliases to approved routes' {
      ...
      Test-WinCareExtensionManifest $manifest|Should -BeTrue
      $manifest.commandFiles=@('commands.ps1')
      {Test-WinCareExtensionManifest $manifest}|Should -Throw '*must be JSON*'
    }
    ...
    It 'bounds image metadata before decoder allocation' {
      ...
      {Get-WinCareImageMetadata -LiteralPath $path}|Should -Throw '*256 MiB*'
    }
    ...
    It 'fails closed on incomplete note pairs and literalizes note queries' {
      ...
      {Get-WinCareLocalNoteSnapshot -Id $id}|Should -Throw '*incomplete*'
