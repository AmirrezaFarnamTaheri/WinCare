...
Describe 'Governed expert convergence contracts' {
  InModuleScope WinCare {
    ...
    It 'fails closed for authority-bearing expert mutations by default' {
      ...
      $p.AllowPageFileDisable|Should -BeFalse
      $p.AllowSecurityControlReduction|Should -BeFalse
      $p.AllowUacDisable|Should -BeFalse
      $p.AllowWindowsUpdateServiceDisable|Should -BeFalse
      $p.AllowNetworkExperiments|Should -BeFalse
      $p.AllowInjectionSurfaceRemediation|Should -BeFalse
    ...
    It 'limits UAC disable to disposable labs and all reductions to expiry records' {
      {New-WinCareSecurityMaintenancePlan -Control Uac -DurationMinutes 30 -Reason 'Compatibility test' -EnvironmentClass Workstation}|Should -Throw '*DisposableLab*'
    ...
    It 'requires Critical risk for page-file and UAC reductions' {
      ...
      (Test-WinCareActionContract -Action $page).Success|Should -BeFalse
      (Test-WinCareActionContract -Action (New-WinCareAction -Type SetPageFileConfiguration -Label 'Disable pagefile' -Risk Critical -Parameters @{Mode='Disabled';Settings=@();BeforeHash=('a'*64)} -RequiresAdmin $true -Reversible $true -TimeoutSeconds 600)).Success|Should -BeTrue
      ...
      (Test-WinCareActionContract -Action $uac).Success|Should -BeFalse
      $uac.Risk='Critical';(Test-WinCareActionContract -Action $uac).Success|Should -BeTrue
      $uac.Parameters.Enabled=$true;(Test-WinCareActionContract -Action $uac).Success|Should -BeFalse
    }
    It 'rejects security recovery snapshots that target unexpected authority paths' {
      ...
      {Test-WinCareSecurityControlSnapshot -Snapshot $payload -ExpectedControl Uac}|Should -Throw '*unexpected Registry target*'
    }
    ...
    It 'rejects injection recovery descriptors outside exact remediable roots' {
      ...
      {Test-WinCareInjectionSurfaceSnapshot -Snapshot $snapshot}|Should -Throw '*IFEO snapshot path is invalid*'
    }
