...
BeforeAll { Import-Module "$((Get-Location).Path)\src\WinCare\WinCare.psd1" -Force -Global }
  InModuleScope WinCare {
    ...
    It 'keeps sensitive operations policy denied by default' {
      $policy=Get-WinCareDefaultPolicy
    ...
    It 'uses exact Steam file manifests and archive hashes' {
      ...
      $sourceFiles=(Get-Command Get-WinCareSteamBackupSourceFile).ScriptBlock.ToString()
      $sourceFiles|Should -Match 'Get-WinCareSha256'
    ...
    It 'routes every operations workspace and headless command' {
    ...
    It 'keeps every operations mutation outside the read-only broker' {
