...
Describe 'Cumulative donor convergence contracts' {
  InModuleScope WinCare {
    ...
    It 'loads every strict widget definition and rejects executable providers' {
      ...
      foreach($widget in $widgets){Test-WinCareWidgetDefinition $widget|Should -BeTrue}
      $bad=@{id='bad-widget';title='Bad';description='';provider='PowerShellScript';minimumRefreshSeconds=1;sourceRecords=@('TEST')}
      {Test-WinCareWidgetDefinition $bad}|Should -Throw '*not approved*'
    }
    ...
    It 'enforces closed maintenance transitions and protected storage' {
      Test-WinCareMaintenanceTransition Scheduled Notice|Should -BeTrue
      Test-WinCareMaintenanceTransition Notice Completed|Should -BeFalse
      Test-WinCareMaintenanceTransition Completed Active|Should -BeFalse
      ...
      Test-WinCareMaintenanceRecord $record|Should -BeTrue
    ...
    It 'uses data-only playbooks that compose existing plans' {
      ...
      foreach($playbook in $playbooks){Test-WinCarePlaybookDefinition $playbook|Should -BeTrue}
    ...
    It 'keeps context-menu mutations inside approved registry roots' {
      foreach($rule in @(Get-WinCareContextMenuRule)){Test-WinCareContextMenuRule $rule|Should -BeTrue}
      ...
      {Test-WinCareContextMenuRule $bad}|Should -Throw '*outside approved roots*'
    }
    It 'blocks offline servicing under safe default policy' {
      (Get-WinCareDefaultPolicy).AllowOfflineImageServicing|Should -BeFalse
      {Test-WinCareOfflineImagePolicy}|Should -Throw '*disabled by policy*'
    }
