...
Describe 'Wave-four operations convergence contracts' {
  InModuleScope WinCare {
    ...
    It 'keeps sensitive wave-four policy denied by default' {
      $policy=Get-WinCareDefaultPolicy
      $policy.AllowInsecureDownloads|Should -BeFalse
      $policy.AllowOfflineImageReduction|Should -BeFalse
      $policy.AllowGameStateRestore|Should -BeFalse
    ...
    It 'rejects insecure download URLs by default' {
      {Test-WinCareDownloadUri 'http://example.test/file.bin'}|Should -Throw '*disabled by policy*'
    ...
    It 'rejects durable sensitive download headers' {
      {Test-WinCareDownloadHeaderMap @{Authorization='Bearer secret'}}|Should -Throw '*not permitted*'
      {Test-WinCareDownloadHeaderMap @{Cookie='secret=value'}}|Should -Throw '*not permitted*'
      Test-WinCareDownloadHeaderMap @{'User-Agent'='WinCare test'}|Should -BeTrue
    }
    ...
    It 'builds a protected download queue action with exact recovery' {
      ...
      (Test-WinCareActionContract $plan.Actions[0]).Success|Should -BeTrue
    }
    ...
    It 'requires runtime-bound cancellation for started downloads' {
      ...
      (Test-WinCareActionContract $action).Success|Should -BeTrue
    ...
    It 'rejects malformed telemetry series' {
      ...
      Test-WinCareTelemetrySeries $series|Should -BeTrue
      $series.Samples[0].CpuPercent=101
      {Test-WinCareTelemetrySeries $series}|Should -Throw '*percentage*'
    }
    ...
    It 'evaluates arithmetic without Invoke-Expression or a shell' {
      ...
      {Invoke-WinCareCalculator 'Get-Process'}|Should -Throw '*Unsupported calculator token*'
    ...
    It 'validates immutable offline reduction profile schemas' {
      foreach($profile in @(Get-WinCareOfflineReductionProfile)){Test-WinCareOfflineReductionProfile $profile|Should -BeTrue}
      $bad=ConvertTo-WinCareParameterDictionary (Get-WinCareOfflineReductionProfile -Id 'nano-safe')
      $bad.AppxDisplayNames=@('*')
      {Test-WinCareOfflineReductionProfile $bad}|Should -Throw '*AppX*'
    }
    ...
    It 'validates workspace layout ratios and unique slots' {
      ...
      Test-WinCareWorkspaceLayoutRecord $layout|Should -BeTrue
      $layout.Slots[0].WidthRatio=1.1
      {Test-WinCareWorkspaceLayoutRecord $layout}|Should -Throw '*ratio*'
    }

    It 'bounds workspace title regex execution' {
      New-WinCareWorkspaceTitleRegex '^Notepad'|Should -Not -BeNullOrEmpty
      {New-WinCareWorkspaceTitleRegex '('}|Should -Throw '*invalid or unsafe*'
