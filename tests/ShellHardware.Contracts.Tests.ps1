...
Describe 'Shell and Hardware Studio contracts' {
    BeforeAll {
    ...
    It 'uses current source ownership instead of historical authority labels' {
        $Provider|Should -Not -Match '[''"](?:D[0-9]{1,3}|W[0-9]+|CG)-[A-Z0-9-]+[''"]'
        $Provider|Should -Match "SourceRecords=@\('SRC:[0-9a-f]{10}'\)"
    }
    ...
    It 'includes Winhance only through the separately authorized unsafe plane' {
        ...
        @($profile.SourceRecords)|Should -Contain 'SRC:1461504853'
