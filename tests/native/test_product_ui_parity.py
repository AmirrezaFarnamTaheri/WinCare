...
class ProductUiParityTests(unittest.TestCase):
    ...
    def test_home_is_presentation_only_and_has_one_primary_checkup_cta(self) -> None:
        ...
        self.assertIn("Your latest checkup is ready", view_model)
        self.assertIn("Some checkup results are getting old", view_model)
        self.assertIn('Text="Checkup coverage"', xaml)
        self.assertIn('Text="Latest results from the four read-only checkup areas."', xaml)
        self.assertNotIn('Text="Evidence collected"', xaml)
        self.assertNotIn("Recent evidence is ready", view_model)
