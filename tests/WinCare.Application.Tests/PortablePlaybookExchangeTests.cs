...
public sealed class PortablePlaybookExchangeTests
{
    ...
    [Theory]
    [InlineData("{\"schemaVersion\":\"1\",\"name\":\"Bad\",\"steps\":[]}")]
    [InlineData("{\"schemaVersion\":1,\"name\":\"Bad\",\"steps\":[{\"commandId\":\"cleaner-disk-pressure\",\"parameters\":{\"OlderThanDays\":\"7\"}}]}")]
    public void Import_wraps_wrong_numeric_types_as_validation_errors(string json)
    {
        Assert.Throws<PortablePlaybookValidationException>(() => PortablePlaybookExchange.Import(json, Catalog));
    }
