...
public sealed class ToolParameterRoundTripTests
{
    ...
    [Fact]
    public void Imported_parameter_values_populate_typed_fields()
    {
        var viewModel = new ToolExecutionViewModel(new CommandDispatcher([CleanerDefinition()], [new NoopHandler()]), _ => { });
        viewModel.SelectTool(new ToolRowViewModel(CleanerDefinition()));

        viewModel.ApplyParameterValues(System.Text.Json.JsonSerializer.SerializeToElement(new { OlderThanDays = 42 }));

        Assert.Equal("42", Assert.Single(viewModel.ParameterFields).Value);
        Assert.Contains("42", viewModel.ParameterJson);
    }
