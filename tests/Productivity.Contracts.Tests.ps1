...
Describe 'Productivity contracts' {
...
BeforeAll { Import-Module "$((Get-Location).Path)\src\WinCare\WinCare.psd1" -Force -Global }
  InModuleScope WinCare {
    ...
    It 'registers and dispatches every productivity typed action' {
      $expected=@(
    ...
    It 'routes every productivity workspace and headless command' {
    ...
    It 'admits only data-only extension command aliases to approved routes' {
      $manifest=[ordered]@{schemaVersion=1;id='test.commands';name='Test Commands';version=$script:WinCareVersion;publisher='Test';description='';minimumWinCareVersion=$script:WinCareVersion;files=@(@{path='commands.json';sha256=('a'*64);bytes=10});catalogFiles=@();knowledgeFiles=@();commandFiles=@('commands.json');sourceRecords=@('EXT:test.commands')}
