...
2. **Rust 2024 Native Engine & Health Guard**: Memory-safe C-ABI FFI core (`wincare-core`) with `std::panic::catch_unwind` and background monitoring daemon (`wincare-guard`) with Named Pipe IPC and Windows Toast notifications.
