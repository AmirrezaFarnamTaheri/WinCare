...
def main()->int:
    ...
    files=sorted(p for p in root.rglob('*') if p.is_file() and p.suffix.lower() in PSEXT and not p.name.startswith('.') and not any(part.startswith('.') for part in p.parts[:-1]) and 'artifacts' not in p.parts and 'tasks' not in p.parts)
    funcs={};refs={}
    for p in files:
    ...
    for p in sorted(root.rglob('*.json')):
        if p.name.startswith('.') or any(part.startswith('.') for part in p.parts[:-1]) or 'artifacts' in p.parts or 'tasks' in p.parts: continue
    ...
    if len(manifest_versions)!=1:
        ...
        for candidate in root.rglob('*'):
            if not candidate.is_file() or candidate.name.startswith('.') or any(part.startswith('.') for part in candidate.parts[:-1]) or 'artifacts' in candidate.parts or 'tasks' in candidate.parts or 'docs' in candidate.parts or candidate.suffix.lower() not in {'.ps1','.psm1','.psd1','.xaml','.cs'}:
                continue
            if candidate.name in {'BUILD-RECEIPT.json','SBOM.spdx.json','release_checklist.py','validate_source.py'}:
                continue
