...
function Read-WinCareStaticText {
    param(
        [Parameter(Mandatory)][string]$Path,
        [ValidateRange(1,67108864)][long]$MaximumBytes=16777216
    )
    Read-WinCareToolingBoundedUtf8Text -LiteralPath $Path -MaximumBytes ([int]$MaximumBytes) -Purpose 'Static-analysis input'
}
...
foreach($file in $files){
    if($file.Attributes -band [IO.FileAttributes]::ReparsePoint){$failures.Add("Unsafe PowerShell source reparse point: $($file.FullName)");continue}
    if([long]$file.Length -gt 16777216L){$failures.Add("PowerShell source exceeds 16 MiB: $($file.FullName)");continue}
...
$placeholderFiles=@($securityFiles)
    ...
    if(Test-Path -LiteralPath $native.Path){
        try{
            if(-not ($native.Type -as [type])){
                $nativeText=(Read-WinCareStaticText -Path $native.Path -MaximumBytes 16777216).Replace('namespace WinCare.Native','namespace WinCare.StaticValidation')
...
if($python){
    foreach($validator in @('validate_source.py','validate_action_bindings.py','validate_test_source_assertions.py','validate_powershell_calls.py','validate_module_manifest.py','validate_source_references.py','audit_stub_inventory.py','validate_maintainability.py','validate_network_egress.py','validate_external_processes.py','validate_bounded_io.py','validate_read_only_state.py')){
        $path=Join-Path $PSScriptRoot $validator
        $validationResult=Invoke-WinCareToolingProcess -Executable $python.Source -Arguments @($path,$rootPath) -TimeoutSeconds 600 -MaximumCapturedOutputBytes 33554432 -WorkingDirectory $rootPath -WriteCapturedOutput
        if($validationResult.ExitCode -ne 0){$failures.Add("Python validator failed: $validator")}
    }
    $testResult=Invoke-WinCareToolingProcess -Executable $python.Source -Arguments @('-m','unittest','tools.test_release_tools','tools.test_maintainability','tools.test_security_invariants','tools.test_cleaner_preview','tools.test_gui','tools.test_source_references','-v') -TimeoutSeconds 900 -MaximumCapturedOutputBytes 33554432 -WorkingDirectory $rootPath -WriteCapturedOutput
    if($testResult.ExitCode -ne 0){$failures.Add('Python release-tool tests failed.')}
}else{$failures.Add('Python 3 is required for source and release validation.')}
