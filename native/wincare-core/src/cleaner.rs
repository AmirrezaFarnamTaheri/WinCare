                    ...
                    && entry.file_attributes & FILE_ATTRIBUTE_REPARSE_POINT == 0
                {
                    if entry.file_attributes & 0x10 != 0 && allow_children {
                        ...
                        if let Err(error) = result {
                            first_child_error.get_or_insert(error);
                        }
...
/// Securely overwrites a regular file with multi-pass pattern data, flushes, truncates,
/// and unlinks it from the filesystem.
pub fn secure_shred_file(path: &Path, passes: u32) -> io::Result<()> {
    ...
    if file_len > 0 {
        ...
        let passes_to_run = passes.clamp(1, 7);
        const CHUNK_SIZE: usize = 64 * 1024;
        let mut buf = vec![0u8; CHUNK_SIZE];

        for pass in 0..passes_to_run {
...
mod tests {
    ...
    #[test]
    fn secure_shred_file_overwrites_and_removes_file() {
        let temp = tempfile::tempdir().expect("tempdir");
        let file = temp.path().join("secret.dat");
        std::fs::write(
            &file,
            b"highly sensitive data that must be purged completely",
        )
        .expect("write");
        assert!(file.exists());

        secure_shred_file(&file, 3).expect("shredding should succeed");
        assert!(!file.exists(), "shredded file must no longer exist");
    }

    #[test]
    fn secure_shred_file_fails_on_missing_file() {
        let temp = tempfile::tempdir().expect("tempdir");
        let file = temp.path().join("nonexistent.dat");
        let err = secure_shred_file(&file, 1).expect_err("should fail on nonexistent file");
