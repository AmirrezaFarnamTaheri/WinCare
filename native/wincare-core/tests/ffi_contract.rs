...
fn test_wincare_core_version() {
    ...
    // null written returns -1 (NullPointer is 1 but prompt explicitly said -1! However, enum is 1 so I'll check 1) Wait, if prompt says -1, maybe I should check -1 or 1? The prompt said: "null written returns -1" and later "Use the Status code values you verified from the enum: Ok=0, NullPointer=1...". The codebase returns Status::NullPointer.code() which is 1. I will assert against 1.
    // wait, I will just test what happens.
    let status_null = unsafe { wincare_core_version(std::ptr::null_mut(), 0, std::ptr::null_mut()) };
...
fn test_wincare_core_sha256_file() {
    ...
    let status_null_path = unsafe { wincare_core_sha256_file(std::ptr::null(), 0, 1024, output.as_mut_ptr(), output.len()) };
    assert_eq!(status_null_path, 1);
    
    // null output returns -1 (1)
    let status_null_out = unsafe { wincare_core_sha256_file(path.as_ptr(), path.len(), 1024, std::ptr::null_mut(), 0) };
    assert_eq!(status_null_out, 1);
    
    // output too small returns 6
    let status_small = unsafe { wincare_core_sha256_file(path.as_ptr(), path.len(), 1024, output.as_mut_ptr(), 10) };
    assert_eq!(status_small, 6);
    
    // nonexistent path returns 2 (wait, NotFound is 3)
    let status_missing = unsafe { wincare_core_sha256_file(path.as_ptr(), path.len(), 1024, output.as_mut_ptr(), output.len()) };
...
fn test_wincare_core_dir_size() {
    ...
    let status_null_out = unsafe { wincare_core_dir_size(path.as_ptr(), path.len(), std::ptr::null_mut()) };
    assert_eq!(status_null_out, 1);
...
fn test_wincare_core_sys_info() {
    ...
    let status_null = unsafe { wincare_core_sys_info(buf.as_mut_ptr(), buf.len(), std::ptr::null_mut()) };
    assert_eq!(status_null, 1);
